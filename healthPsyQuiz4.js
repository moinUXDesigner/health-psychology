
    // ---------- Week 1 Questions ----------
const questions1 = [
    {
        q: "Health psychology has been recognized within the American Psychological Association as-",
        options: ["Division 38", "Division 36", "Division 43", "Division 48"],
        // correct a: Division 38
        a: 0
    },
    {
        q: "Emotion is accompanied by physiological changes- This finding was put forward by",
        options: ["Franz", "Dunbar", "Freud", "Cannon"],
        // correct a: Cannon
        a: 3
    },
    {
        q: "One of these is currently not an area of activity for health psychology-",
        options: ["Prevention of illness", "Prescription of medicines", "Treatment of illness", "Promotion of health"],
        // correct a: Prescription of medicines
        a: 1
    },
// =================================================================================================
  {
    type: "match",
    q: "Match List I with List II",
    list1: [
      "A. Experimental method",
      "B. Surveys",
      "C. Case series",
      "D. Naturalistic observation"
    ],
    list2: [
      "I. Mostly preferred for measuring attitudes",
      "II. Cause and effect relationship",
      "III. No interference of the researcher",
      "IV. Triangulation"
    ],
    options: [
            "A- III, B- IV, C- II, D- I",
            "A- III, B- I, C- IV, D- II",
            "A- III, B- IV, C- I, D- II",
            "A- II, B- I, C- IV, D- III"
        ],
        // correct mapping: A- II (experimental => cause & effect), B- I (surveys=> attitudes), C- IV (case series => triangulation), D- III (naturalistic => no interference)
    a: 3

  },
  // =================================================================================================

    // {
    //     q: `Match List I with List II\n\nList I:\nA. Experimental method\nB. Surveys\nC. Case series\nD. Naturalistic observation\n\nList II:\nI. Mostly preferred for measuring attitudes\nII. Cause and effect relationship\nIII. No interference of the researcher\nIV. Triangulation`,
    //     options: [
    //         "A- III, B- IV, C- II, D- I",
    //         "A- III, B- I, C- IV, D- II",
    //         "A- III, B- IV, C- I, D- II",
    //         "A- II, B- I, C- IV, D- III"
    //     ],
    //     // correct mapping: A- II (experimental => cause & effect), B- I (surveys=> attitudes), C- IV (case series => triangulation), D- III (naturalistic => no interference)
    //     a: 3
    // },
    {
        q: "In the development of physical disease symptoms, Sigmund Freud’s emphasized on the role of",
        options: ["Unconscious physical factors", "Unconscious psychological factors", "Conscious psychological factors", "Conscious physical factors"],
        // correct: Unconscious psychological factors
        a: 1
    },
    {
        q: "Psychological processes can complicate diagnostic procedures. This statement is-",
        options: ["Partly False", "True", "Partly True", "False"],
        // correct: True
        a: 1
    },
    {
        q: "Identify which of the following statement is False. Behavioral health emphasizes more on",
        options: ["The prevention of disease", "The enhancement of health", "Adoption of healthy lifestyles", "Better hospital care"],
        // correct: Better hospital care (behavioral health is more public health / prevention oriented)
        a: 3
    },
    {
        q: "The patient receives less information and plays less of a role in decision-making in the case of-",
        options: ["The Patient -centered approach", "The Doctor-centered approach", "The Psychiatrist-centered approach", "The Family-centered approach"],
        // correct: The Doctor-centered approach
        a: 1
    },
    {
        q: "Epidemiological methods are more likely to be used by",
        options: ["Educational Psychologists", "Social Psychologists", "Health Psychologists", "Industrial Psychologists"],
        // correct: Health Psychologists
        a: 2
    },
    {
        q: "Physicians supporting the psychosomatic approach call for an acceptance of the",
        options: ["Psychosocial model", "Biopsychosocial model", "Biosocial model", "Biomedical model"],
        // correct: Biopsychosocial model
        a: 1
    }
];
// ---------- Week 2 Questions ----------
const questions2 = [
    {
        // type: "match",
        // q: "Match List I with List II \n\nA. Health belief model \nB. Protection motivation theory \nC. Theory of reasoned action \nD. Theory of planned behaviour \n\nList II: \nI. Coping appraisal \nII. Perceived behavioural control \nIII. Perceived threat \nIV. Attitude & subjective norms",
        type: "match",
        q: "Match List I with List II",
        list1: [
            "A. Health belief model",
            "B. Protection motivation theory",
            "C. Theory of reasoned action",
            "D. Theory of planned behaviour"
        ],
        list2: [
            "I. Coping appraisal",
            "II. Perceived behavioural control",
            "III. Perceived threat",
            "IV. Attitude & subjective norms"
        ],
        options: [
            "A- III, B- I, C- IV, D- II",
            "A- IV, B- II, C- I, D- III",
            "A- I, B- III, C- II, D- IV",
            "A- II, B- I, C- III, D- IV"
        ],                
        a: 0 // Correct: A-III, B-I, C-IV, D-II
    },
    {
        q: "Health is ______________ in nature.",
        options: ["Multi-factorial", "Multi-sectorial", "Multi-dimensional", "Multi-cultural"],
        a: 2 // Multi-dimensional
    },
    {
        q: "Which of the following theory supports the statement “While the physical and social environment shapes behaviour, people are not passive in the process, since they in turn can change their environments, creating a reciprocal dynamic”?",
        options: ["Social learning theory", "Health belief model", "Theory of reasoned action", "Protection motivation theory"],
        a: 0 // Social learning theory
    },
    {
        q: "Which of the following theory includes coping appraisal and threat appraisal?",
        options: ["Health belief model", "Social learning theory", "Theory of reasoned action", "Protection motivation theory"],
        a: 3 // Protection motivation theory
    },
    {
        q: "Medical students' disease is best described by which of the following?",
        options: [
            "Believing that you might have the symptoms of each disease that one studies.",
            "It is not contagious",
            "A disease that only occurs to medical students because they are around sick people all day.",
            "Can occur in anyone who studies about illnesses because all that studying makes everyone ill."
        ],
        a: 0 // Believing they have every disease they study
    },
    {
        q: "The existence of feedback loops contributes to Leventhal's model being referred to as______.",
        options: ["Self-monitoring", "Self-regulatory", "Self-reporting", "Self-perceiving"],
        a: 1 // Self-regulatory
    },
    {
        q: "Which of the following terms best describes an individual who express distress and conflict through bodily symptoms?",
        options: ["Natural healers", "Pessimist", "Somaticizers", "Worried well"],
        a: 2 // Somaticizers
    },
    {
        q: "Identify four categories offered by Lalonde which are helpful in tracking any health problem?",
        options: [
            "Environment, Lifestyle, Health care organisation and Economy of the country.",
            "Human biology, Mental health, Hygiene practices and Lifestyle.",
            "Mental health, Health care organization, economy of the country & Lifestyle.",
            "Human biology, Environment, Lifestyle and Health care organisation."
        ],
        a: 3 // Human biology, Environment, Lifestyle, Health care organisation
    },
    {
        q: "The epidemiological triangle portrays the interaction between__",
        options: ["Individual, host and environment", "Agent, individual and host", "Agent, host and environment", "Individual, disease and environment"],
        a: 2 // Agent, host and environment
    },
    {
        q: "What are the five consistent themes of 'illness representations' as identified by Leventhal?",
        options: [
            "Identity, consequences, coping, timeline, and curability/controllability",
            "Identity, consequences, cause, coping, and curability/controllability",
            "Identity, consequences, cause, timeline, and curability/controllability",
            "Identity, coping, cause, timeline, and curability/controllability"
        ],
        a: 2 // Identity, consequences, cause, timeline, curability/controllability
    }
];
// ---------- Week 3 Questions ----------
const questions3 = [
    {
        q: "_________ provides an active learning environment for audiences and encourages the exploration of social attitudes towards particular health issues.",
        options: ["Mass media appeal", "Appeal by physicians", "Theatrical performances", "Radio"],
        a: 2 // Theatrical performances
    },
    {
        q: "Brownell et al., stated that in order to really succeed at long-term maintenance and control, an individual must become aware of his/her own potentially ______ and learn to use self-instructions that are compatible with success.",
        options: ["Encouraging positive thoughts", "Supportive member of the family or friends", "Damaging negative self-statements", "Destructive behaviors"],
        a: 2 // Damaging negative self-statements
    },
    // ====================================================CHECK============================================
    {
        type: "alphalist",
        q: "Which of the following statements is False regarding self-regulatory model of illness cognition?",
        list1: [
            "A. This model is developed by Leventhal.", 
            "B. It suggests that illness is dealt by individuals in the same way as other problems.",
            "C. This model has three stages.", 
            "D. Its major assumption is that the role individual with illness is passive."
        ],
        
        options: ["D only", "C & D only", "A & B only", "B only"],
        a: 0 // D only
    },
    // ================================================================================================
    {
        q: "Which of the following is Not an example of Approach coping?",
        options: ["Resting", "Denial of illness", "Taking medicines", "Visiting doctor"],
        a: 1 // Denial of illness
    },
    {
        q: "Which theory proposes that reinforcement and observation of others' behaviour causes change in the behaviour?",
        options: ["Cognitive Social Learning", "Transtheoretical Model", "Social Action", "Health Belief Model"],
        a: 0 // Cognitive Social Learning
    },
    {
        q: "Identify the three stages of Leventhal’s self-regulatory model of illness cognition from below:",
        options: ["Involvement, appraisal & change in behaviour", "Interpretation, coping & appraisal", "Knowing, altering beliefs and coping", "Acquiring information, interpretation & change in behaviour"],
        a: 1 // Interpretation, coping & appraisal
    },
    {
        q: "Operant behaviour that is rewarded or reinforced by the individual or by the environment is more likely to be _________________",
        options: ["Reduce gradually", "Relapse", "Repeat", "Fluctuate"],
        a: 2 // Repeat
    },
    {
        q: "Assertion A: Central arguments require an analytically motivated audience.\nReason R: Peripheral route persuasion is not analytical and requires less effort on the part of the audience.",
        options: [
            "A is not correct but R is correct",
            "A is correct but R is not correct",
            "Both A and R are correct but R is NOT the correct explanation of A",
            "Both A and R are correct and R is the correct explanation of A"
        ],
        a: 2 // Both A and R are correct but R is NOT the correct explanation of A
    },
    {
        q: "____________ are useful tools for forming mental constructs that simplify or render familiar a concept that the individual is attempting to understand.",
        options: ["Analogies", "Life skills", "Mass media appeals", "Puppetry"],
        a: 0 // Analogies
    },
    {
        q: "Behavioural Intention =",
        options: [
            "Control beliefs + Will power",
            "Normative belief + Persuasion",
            "Attitude + Persuasion",
            "Attitude + Subjective norms"
        ],
        a: 3 // Attitude + Subjective norms
    }
];
// ---------- Week 4 Questions ----------
const questions4 = [
    {
        type: "match",
        q: "Match List I with List II",
        list1: ["A. Eustress", "B. Distress", "C. Catastrophes", "Major Life Events"],
        list2: ["I. Negative stress", "II. Stress caused by death of a loved one", "III. Positive stress", "IV. Unpredictable stressor such as war"],
        options: [
            "A- III , B- I , C- IV , D- II",
            "A- III, B- IV, C- II, D- I",
            "A- IV, B- III, C- I, D- II",
            "A- I, B- IV, C- II, D- III"
        ],
        a: 0 // A-III, B-I, C-IV, D-II
    },
    {
        q: "Vasopressin and oxytocin are hormones secreted by the",
        options: ["Anterior pituitary gland", "Posterior pituitary gland", "Pineal gland", "Thymus gland"],
        a: 1 // Posterior pituitary gland
    },
    {
        q: "According to General Adaptation Syndrome model, at which stage ‘disease of adaptation’ takes place?",
        options: ["Exhaustion stage", "Resistance stage", "Crises stage", "Alarm stage"],
        a: 1 // Resistance stage
    },
    {
        q: "Selye’s General Adaptation Syndrome is known as a-",
        options: [
            "Transactional Model of Stress",
            "Interactional Model of Stress",
            "Psychological Model of Stress",
            "Biological Model of Stress"
        ],
        a: 3 // Biological Model of Stress
    },
    {
        q: "Which of the following is Not an alternative medicine for alcohol abuse?",
        options: ["Acupuncture", "Yoga", "Meditation", "Injected medication"],
        a: 3 // Injected medication
    },
    {
        q: "The Perceived Stress Scale does NOT assess one of these-",
        options: ["Daily hassles", "Changes in state anxiety", "Changes in coping resources", "Major events"],
        a: 1 // Changes in state anxiety
    },
    {
        q: "Adrenocorticotropic hormone (ACTH) is a hormone secreted by the-",
        options: ["Thymus gland", "Thyroid gland", "Adrenal gland", "Pituitary gland"],
        a: 3 // Pituitary gland
    },
    {
        q: "The activation of this system is responsible for restoring physiological balance after a stress response-",
        options: [
            "The Autonomic Nervous System",
            "The Parasympathetic Nervous System",
            "The Sympathetic Nervous System",
            "The Peripheral Nervous System"
        ],
        a: 1 // Parasympathetic Nervous System
    },
    {
        q: "Individuals who begin drinking at _____ age are at a higher risk of problem drinking or physical dependence on alcohol.",
        options: ["A later", "A late", "A middle", "An early"],
        a: 3 // An early
    },
    {
        q: "The level of secretion of catecholamines is a ___________",
        options: ["Biochemical Measure of stress", "Psychological Measure of stress", "Psychosocial Measure of stress", "Physiological Measure of stress"],
        a: 0 // Biochemical Measure of stress
    }
];
// ---------- Week 5 Questions ----------
const quizQuestions5 = [
    {
        q: "The influence of chronic stress on gastric functioning causes-",
        options: [
            "Increased production of gastric acid",
            "Increased production of cortisol",
            "Reduced production of gastric acid",
            "Increased sympathetic activity"
        ],
        a: 0 // Increased production of gastric acid
    },
    {
        q: "Assertion A: Emotional factors especially chronic stress can influence the functioning of the stomach.\nReason R: The stress response involves release of cortisol and glucocorticoids which can act as causative as also aggravating factors for peptic ulcerations.",
        options: [
            "Both A and R are correct and R is the correct explanation of A",
            "Both A and R are correct but R is NOT the correct explanation of A",
            "A is correct but R is not correct",
            "A is not correct but R is correct"
        ],
        a: 0
    },
    {
        q: "Focussing on the arrangement of a clinical setting, rather than the pain during a medical procedure is an example of which of the following cognitive control technique?",
        options: [
            "Thought stopping",
            "Thought avoidance",
            "Informational control",
            "Cognitive restructuring"
        ],
        a: 3 // Cognitive restructuring
    },
    {
        q: "A moderate ambition and drive are characteristics of –",
        options: [
            "Type B personality",
            "Type D personality",
            "Type C personality",
            "Type A personality"
        ],
        a: 0 // Type B personality
    },
    {
        q: "The two themes of perfectionism are ____________ and _____________",
        options: [
            "Perfectionist striving; Perfectionist concern",
            "Third-person perfectionism; Socially prescribed perfectionism",
            "Self-oriented perfectionism; Perfectionist concern",
            "Other-oriented perfectionism; Perfectionist striving"
        ],
        a: 0
    },
    {
        q: "Residual coping occurs ________",
        options: [
            "Just before the stressful event",
            "Long before the stressful event",
            "Just after the stressful event",
            "Long after the stressful event"
        ],
        a: 3 // Long after the stressful event
    },
    {
        q: "Expressing pent-up feelings in the company of family and friends is an example of ______",
        options: [
            "Problem-focused coping",
            "Avoidance coping",
            "Expressive coping",
            "Emotion-focused coping"
        ],
        a: 3 // Emotion-focused coping
    },
    {
        q: "Those possessing _____________ tend to see a stressful situation as beyond their control.",
        options: [
            "An external locus of control",
            "Prior experience",
            "Internal locus of control",
            "High self-efficacy"
        ],
        a: 0
    },
    {
        q: "Statement I: Type A people are emotional, achievement oriented, competitive, time conscious, perfectionist and are always functioning above their physiological baseline.\n\nStatement II: Type D individuals are often worried without specific reasons, have a pessimistic perspective, often feel depressed and irritated, and rarely experience positive feelings.",
        options: [
            "Statement I is correct but Statement II is incorrect",
            "Both Statement I and Statement II are incorrect",
            "Both Statement I and Statement II are correct",
            "Statement I is incorrect but Statement II is correct"
        ],
        a: 2
    },
    {
        q: "One of these is not a part of the Big Five model of personality _____",
        options: [
            "Conscientiousness",
            "Extraversion",
            "Psychoticism",
            "Neuroticism"
        ],
        a: 2 // Psychoticism
    }
];
// ---------- Week 6 Questions ----------
const quizQuestions6 = [
    {
        q: "Which of the following is Not one of the three stages of stress inoculation?",
        options: [
            "Skill Acquisition and Rehearsal",
            "Relaxation",
            "Conceptualization",
            "Application and Follow-through"
        ],
        a: 1
    },
    {
        q: "Confused thinking is associated with",
        options: [
            "Delirium",
            "Schizophrenia",
            "Eating disorder",
            "Both a and c"
        ],
        a: 3 // Both a and c
    },
    {
        q: "This phase occurs between the phases of ‘medical care contact’ and ‘dependent patient role’",
        options: [
            "Sick-role phase",
            "Chronic phase",
            "Acute phase",
            "Validation phase"
        ],
        a: 2 // Acute phase
    },
    {
        q: "Functional measurements of social support analyze",
        options: [
            "The interdependence of social contacts",
            "The tasks undertaken by social networks",
            "The number of social contacts",
            "The interlinkages between social networks"
        ],
        a: 1
    },
    {
        q: "Illness is more often associated with –",
        options: [
            "Positive outcomes",
            "Long-term outcomes",
            "Negative outcomes",
            "Chronic outcomes"
        ],
        a: 2
    },
    {
        q: "________ helps an individual gain information about changes in biological responses as they are taking place.",
        options: [
            "Progressive muscle relaxation technique",
            "Mindfulness meditation",
            "Stress inoculation technique",
            "Biofeedback"
        ],
        a: 3
    },
    {
        q: "This form of social support is most important for alleviating geriatric depression",
        options: [
            "Informational Social Support",
            "Procedural Social Support",
            "Tangible Social Support",
            "Emotional Social Support"
        ],
        a: 3
    },
    {
        q: "The physical and psychological symptoms of disease may act as causative factors for each other. This statement is",
        options: [
            "False",
            "Partly false",
            "Partly true",
            "True"
        ],
        a: 3
    },
    {
        q: "Edward Suchman proposed",
        options: [
            "Four stages of illness",
            "Five stages of illness",
            "Six stages of illness",
            "Seven stages of illness"
        ],
        a: 1
    },
    {
        q: "Assertion A: Biofeedback is used as stress management technique.\nReason R: Electromyography-produced electrical activity allows the individual to alter physiological responses.",
        options: [
            "A is correct but R is not correct",
            "Both A and R are correct but R is NOT the correct explanation of A",
            "A is not correct but R is correct",
            "Both A and R are correct and R is the correct explanation of A"
        ],
        a: 1
    }
];
// ---------- Week 7 Questions ----------
const quizQuestions7 = [
    {
        q: "Which of the following would be a characteristic of a chronic illness?",
        options: [
            "Requires short term surveillance",
            "Reversible pathology",
            "Slow, progressive decline",
            "Cure likely"
        ],
        a: 2
    },
    {
        q: "The delay between the making of an appointment with the doctor and actually receiving medical care is –",
        options: [
            "Utilization delay",
            "Illness delay",
            "Appraisal delay",
            "Medical delay"
        ],
        a: 3
    },
    {
        q: "Accuracy in symptom perception is –",
        options: [
            "A multi-dimensional construct",
            "A three-dimensional construct",
            "A two-dimensional construct",
            "A unidimensional construct"
        ],
        a: 0
    },
    {
        q: "During the breaking of the bad news the time pressure affects ____",
        options: [
            "All the above",
            "The Family Members",
            "The Patient",
            "The Physician"
        ],
        a: 0
    },
    {
        q: "The outcomes of conveying the bad news to the patient depend on ________",
        options: [
            "Multiple variables",
            "Three variables",
            "Two variables",
            "One variable"
        ],
        a: 0
    },
    {
        q: "Type A individuals are more likely to report symptoms when –",
        options: [
            "Engaged in a task",
            "Regulating a task",
            "Just before a task",
            "Not engaged in a task"
        ],
        a: 3
    },
    {
        q: "Rita who is suffering from diabetes is busy in the preparations for her sister’s wedding. She is ______",
        options: [
            "More likely to notice the symptoms",
            "More likely to exaggerate the symptoms",
            "Both a and c.",
            "Less likely to notice the symptoms"
        ],
        a: 3
    },
    {
        q: "A fear of being wrong and looking silly if the symptom is trivial, is less, when the individual –",
        options: [
            "has to depend on others for medical aid",
            "has to disturb others during night-time",
            "has to apply for leave on a busy day",
            "can seek medical help independently"
        ],
        a: 3
    },
    {
        type: "alphalist",
        q: "Given below are two statements: In the light of the statements, choose the most appropriate answer from the options:",
        list1: ["Statement I: SPIKES is a six-step protocol which is used by the clinicians to share bad news with the patients.", "Statement II: ABCDE is a stepwise model which makes the process of breaking the bad news more systematic and easier."],
        options: [
            "Both Statement I and Statement II are correct",
            "Statement I is incorrect but Statement II is correct",
            "Both Statement I and Statement II are incorrect",
            "Statement I is correct but Statement II is incorrect"
        ],
        a: 2
    },
    {
        q: "A condition that interferes with daily functioning for more than 3 months in a year, causes hospitalization of more than 1 month in a year, or is likely to do either of these is termed as:",
        options: [
            "Handicap",
            "Chronic Illness",
            "Lifetime Impairment",
            "Disability"
        ],
        a: 1
    }
];
// ---------- Week 8 Questions ----------
const quizQuestions8 = [
    {
        q: "Support groups may be initiated by-",
        options: [
            "The Therapist",
            "Neither a or b",
            "The Patient",
            "Either a or b"
        ],
        a: 3
    },
    {
        q: "Rehabilitation professionals who help patients relearn daily routines or prepare for employment are referred as _____",
        options: [
            "Dieticians",
            "Occupational Therapists",
            "Physiotherapists",
            "Speech Therapists"
        ],
        a: 1
    },
    {
        type: "alphalist",
        q: "Given below are two statements: In the light of the statements, choose the most appropriate answer from the options:",
        list1: ["Statement I: In most cultures, women are found to be more engaged in reproductive labour as compared to productive labour.", "Statement II: While the biological consequences of illness are similar for men and women, the social demands and implications of these consequences make the illness experience far worse for women as compared to men."],
        options: [
            "Statement I is incorrect but Statement II is correct",
            "Both Statement I and Statement II are incorrect",
            "Statement I is correct but Statement II is incorrect",
            "Both Statement I and Statement II are correct"
        ],
        a: 3
    },
    {
        q: "Steps to promote adherence in chronically ill patients may include-",
        options: [
            "Patient Punishment",
            "Cognitive and behavioral interventions",
            "Both a and b",
            "Patient Education"
        ],
        a: 2 // Both a and b
    },
    {
        q: "When the chronically ill child expresses his/her emotions, it is most important to ___",
        options: [
            "Guide",
            "Listen",
            "Advice",
            "Counter"
        ],
        a: 1
    },
    {
        q: "Children express their feelings only verbally. This statement is-",
        options: [
            "True",
            "False",
            "Partly False",
            "Partly True"
        ],
        a: 1
    },
    {
        q: "Which of these statements do not apply to gender? Identify",
        options: [
            "It is a relative concept",
            "It is a social construct",
            "It is a biological construct",
            "It is a dynamic concept"
        ],
        a: 2
    },
    {
        q: "Longevity research indicates that across cultures-",
        options: [
            "Both have equal lifespan",
            "Neither of the above",
            "Men outlive women",
            "Women outlive men"
        ],
        a: 3
    },
    {
        q: "Evaluation of psychological aspects should form a-",
        options: [
            "Auxiliary part of Chronic Care",
            "Standard part of Chronic Care",
            "Separate part of Chronic Care",
            "Intermittent part of Chronic Care"
        ],
        a: 1
    },
    {
        type: "alphalist",
        q: "Which of the following helps child cope with chronic illness? Choose the correct answer using the options given below:",
        list1: ["A) Talk openly and honestly with the child", "B) Encourage the child to continue everyday activities", "C) Recognize and respond to the emotional impact of the illness", "D) Providing whatever child asks for without conditions"],
        options: [
            "B & D",
            "A, B & C",
            "A, C & D",
            "C & D"
        ],
        a: 1
    }
];

// ---------- Week 9 Questions ----------
const quizQuestions9 = [
    {
        q: "Arthritis does not affect this area of the body:",
        options: [
            "Small joints",
            "Knees",
            "Wrists",
            "Stomach"
        ],
        a: 3
    },
    {
        q: "Fractures become more common in old age due to-",
        options: [
            "Loss in bone density",
            "Increase in bone size",
            "Both b and c",
            "Increase in Muscle mass"
        ],
        a: 0
    },
    {
        q: "What is not right about Alzheimer’s disease?",
        options: [
            "depression",
            "impairment of language and comprehension",
            "good abstract thinking and judgement levels",
            "impairment of memory and attention"
        ],
        a: 2
    },
    {
        q: "What is not true about Alzheimer’s disease?",
        options: [
            "There is memory loss and other intellectual abilities to weaken",
            "It is a common form of dementia",
            "This condition is generally seen in people who are 65 years and older",
            "The condition is a normal part of aging"
        ],
        a: 3
    },
    {
        q: "In _______ the stiffness is worse after rest, such as the first thing in the morning, and often lasts at least 30 minutes or more.",
        options: [
            "Chronic fatigue syndrome",
            "Diabetes",
            "Alzheimer’s disease",
            "Rheumatoid Arthritis"
        ],
        a: 3
    },
    {
        q: "One of these does not constitute a healthy lifestyle change for the elderly. Identify.",
        options: [
            "Smoking cessation",
            "Reduced alcohol intake",
            "Taking over the counter drugs",
            "Regular doctor visits"
        ],
        a: 2
    },
    {
        q: "Which of the following stage symptoms of Alzheimer’s are mistaken for being caused due to stress or ageing?",
        options: [
            "Early stage",
            "Pre-dementia",
            "Advanced",
            "Moderate"
        ],
        a: 1
    },
    {
        q: "An appropriate teaching measure for the patient with diabetes mellitus related to care of the feet is to ________",
        options: [
            "Avoid softening lotions and creams",
            "Use heat to increase blood supply",
            "Use iodine to disinfect cuts and abrasions",
            "Inspect all surfaces of the feet daily"
        ],
        a: 3
    },
    {
        q: "The nurse assists the patient with nutritional therapy of diabetes with the knowledge that a “diabetic diet” is designed",
        options: [
            "For use during periods of high stress",
            "To help normalize blood glucose through a balanced diet",
            "To normalize blood glucose by elimination of sugar",
            "To be used only for type 1 diabetes"
        ],
        a: 1
    },
    {
        q: "______ has the tendency to affect the larger weight bearing joints:",
        options: [
            "Chronic fatigue syndrome",
            "Diabetes Mellitus",
            "Rheumatoid Arthritis",
            "Osteoarthritis"
        ],
        a: 3
    }
];

// ---------- Week 10 Questions ----------
const quizQuestions10 = [
    {
        q: "Which of these is not a treatment option for Cancer?",
        options: ["Relaxation techniques", "Chemo therapy", "Surgery", "Radiation"],
        a: 0
    },
    {
        q: "Pain can be described in terms of 3 components. They are____",
        options: [
            "Sensory, psychological, and cognitive",
            "Sensory, motivational, and cognitive",
            "Sensory, perceptive, and cognitive",
            "Sensory, motivational, and perceptive"
        ],
        a: 1
    },
    {
        q: "Ancient Greeks believed that pain was associated with ___",
        options: ["Mummies", "Death", "Sin", "Pleasure"],
        a: 3
    },
    {
        q: "Which of these is not a treatment option for Cancer?",
        options: ["Surgery", "Radiation", "Relaxation techniques", "Chemo therapy"],
        a: 2
    },
    {
        q: "A patient has primary lung cancer. Which of the following is the least likely carcinogen responsible?",
        options: ["Radon gas", "Tobacco smoke", "UV radiation", "Asbestos"],
        a: 2
    },
    {
        q: "Which theory proposed that there is no separate system for perceiving pain?",
        options: ["Gate Control Theory", "Specificity theory", "Pattern theory", "Pathophysiology of acute pain theory"],
        a: 2
    },
    {
        q: "Clinicians and researchers have found that a few individuals are more prone to coronary heart disease due to their personality make-up. These individuals are termed as:",
        options: ["Type D individuals", "Type C individuals", "Type B individuals", "Type A individuals"],
        a: 3
    },
    {
        q: "The Type D personality is very frequently termed to as ____________ personality.",
        options: ["Disturbed", "Difficult", "Distressed", "Dangerous"],
        a: 2
    },
    {
        q: "Chronic pain sometimes persists despite the fact that the",
        options: ["Person is sleeping", "Patient took medicines", "Acute pain is there", "Injury has healed"],
        a: 3
    },
    {
        q: "Which fibers create a hypothetical gate that can open and close?",
        options: ["Spinal cord fibers", "Small fibers", "Large fibers", "Brain fibers"],
        a: 2
    }
];

// ---------- Week 11 Questions ----------
const quizQuestions11 = [
    {
        q: "Hospice care is limited to the last",
        options: ["Visit to the hospital", "1 1⁄2 years of life", "6 months of life", "2 years of life"],
        a: 2
    },
    {
        q: "Palliative care is a form of medical treatment focused on _____",
        options: ["complete treatment without relapse", "providing a cure", "not doing any intervention", "Slowing the disease's progress"],
        a: 3
    },
    {
        q: "Pharmacological drugs for treatment of pain do not include",
        options: ["Opioid drugs", "Adjuvant drugs", "Hallucinogens", "Non-opioid drugs"],
        a: 2
    },
    {
        q: "Five Powers to face the thought of death is put forth by ______",
        options: ["Christians", "Hindus", "Jews", "Buddhists"],
        a: 3
    },
    {
        q: "In which method one is connected to electrical sensors that help receive information about the body?",
        options: ["Hypnotherapy", "Biofeedback", "Psychotherapy", "Massage"],
        a: 1
    },
    {
        q: "There are ______types of dying trajectories?",
        options: ["Six", "Two", "One", "Three"],
        a: 3
    },
    {
        q: "Systematic desensitization technique results in _____",
        options: ["Increases anxiety", "Reduces lifestyle diseases", "Helps for learning disabilities", "Overcome fear of death"],
        a: 3
    },
    {
        q: "“If I’m allowed to live, I promise....” represents which stage of death, according to Kubler-Ross?",
        options: ["Promise stage", "Denial stage", "Bargaining stage", "Depression stage"],
        a: 2
    },
    {
        q: "In which of the following methods, nerve transmission to an area is blocked through injection of an agent",
        options: ["Neural Blockade", "Biofeedback", "Surgical lesions", "Neuropathy"],
        a: 0
    },
    {
        q: "The last stage of dying according to Ross is",
        options: ["Depression", "Acceptance", "Denial", "Bargaining"],
        a: 1
    }
];

// ---------- Week 12 Questions ----------
const quizQuestions12 = [
    {
        q: "The emotional response to that loss of a loved one is",
        options: ["Bereavement", "Grief", "Trauma", "Loss"],
        a: 1
    },
    {
        q: "The outward expression of that grief, which may include cultural and religious customs surrounding death is",
        options: ["Grief", "Obituary", "Mourning", "Bereavement"],
        a: 2
    },
    {
        q: "Given below are two statements:\n\nAssertion A: Studies show that children go through a series of stages in their understanding of death.\nReason R: Preschool children usually see death as reversible, temporary, and impersonal.\n\nIn the light of the above statements, choose the most appropriate answer from the options",
        options: [
            "A is correct but R is not correct",
            "A is not correct but R is correct",
            "Both A and R are correct but R is NOT the correct explanation of A",
            "Both A and R are correct and R is the correct explanation of A"
        ],
        a: 3
    },
    {
        q: "In the acronym SIMPLE, S stands for _______",
        options: ["Suggesting the regimen", "Specifying the regimen", "Supportive to the patient", "Simplify the regimen"],
        a: 3
    },
    {
        q: "When children question 'Why did they die?' Parents should say",
        options: ["The person is very good", "God took the person to Him", "The person is very bad", "The person's body couldn't fight the sickness any more"],
        a: 3
    },
    {
        q: "The SIMPLE strategy is practiced by the ________",
        options: ["Practitioner", "Family members of the patient", "Pharmacist", "Patient"],
        a: 0
    },
    {
        q: "Which of the following is a compliant behaviour?",
        options: ["Taking medication", "Self-medication", "Smoking", "Maintaining under weight"],
        a: 0
    },
    {
        q: "Which of the following measures of compliance is objective by nature?",
        options: ["Asking practitioner", "Asking others", "Monitoring medicine usage", "Asking patient"],
        a: 2
    },
    {
        q: "Acknowledgment of the objective fact that one has experienced a death is",
        options: ["Trauma", "Bereavement", "Grief", "Loss"],
        a: 1
    },
    {
        q: "Between the ages of 5 and 9, most children are beginning to realize that death is",
        options: ["Temporary", "Unrealistic", "Reversible", "Final"],
        a: 3
    }
];

// ---------- Week 13 Questions ----------
const quizQuestions13 = [
    {
        q: "The patient’s belief that the treatment will have negative effect, even if the Medication given is completely inert is termed as:",
        options: ["Side effect", "Nocebo effect", "Placebo effect", "Both a & c"],
        a: 1
    },
    {
        q: "Which of the following is one of the central concepts in operationalizing the construct of patient centeredness?",
        options: ["Patient involvement in the decision making and planning of treatment", "Medical outcomes cannot be understood by a layperson", "Professional discussions and not discussions with patients contribute to treatment decisions", "Treatment decisions need not involve patients concerns"],
        a: 0
    },
    {
        q: "In their hypothetico-deductive model of problem solving, Newell and Simon differentiated between two types of plans:",
        options: ["Diagnosis and clinical correlation", "Problem statement and strategic planning", "Heuristics and algorithms", "Logic and technology"],
        a: 2
    },
    {
        q: "The most important information should be given early or late in the flow of information to maximize ____________________, and its importance should be emphasized.",
        options: ["Primacy and recency effect", "Information effects", "Channels of interaction", "Communication effect"],
        a: 0
    },
    {
        q: "Who experimented on placebo treatment during World War II ?",
        options: ["John Hayghart", "Saraphino", "Henry Beecher", "Wright"],
        a: 2
    },
    {
        q: "_________ can be used to help individuals deal with their feelings or to reflect on these feelings, whereas ___________ enables individuals to learn how the process or the intervention will actually be done.",
        options: [
            "Coping skills information; Behavioural instructions",
            "Sensory information; Procedural information",
            "Procedural information; Coping skills information",
            "Behavioural instructions; Sensory information"
        ],
        a: 1
    },
    {
        q: "The _______________ suggested that patients’ beliefs are important and emphasized the patient’s locus of control, perceived social support and the disruption of lifestyle involved in adherence.",
        options: ["Adherence model", "Information model", "Layman’s model", "Communication model"],
        a: 0
    },
    {
        q: "An inert substance with no active medical properties is called",
        options: ["Surgery", "Saline solution", "Morphine", "Paracetamol"],
        a: 1
    },
    {
        q: "Most meetings between health professionals and patients are time-limited and consequently ____________ play a central role in developing and testing a hypothesis and reaching a management decision.",
        options: ["Communication", "Adherence", "Compliance", "Stereotypes"],
        a: 3
    },
    {
        q: "What is it called when both the experimenter and the subject are unaware of a particular placebo treatment?",
        options: ["Uncontrolled clinical trial", "Conditioning effect", "Double blind trial", "Randomized clinical trial"],
        a: 2
    }
];

const quizQuestions14 = [
    {
        q: "A definite underlying pathology is a characteristic of –",
        options: [
            "Disease",
            "Illness",
            "Illness Behavior",
            "Health Behavior"
        ],
        a: 0 // Disease
    },
    {
        q: "Health psychology has been recognized within the American Psychological Association as",
        options: [
            "Division 36",
            "Division 43",
            "Division 38",
            "Division 48"
        ],
        a: 2 // Division 38
    },
    {
        q: "Illness behavior and the assumption of a sick role can often be rewarding. This statement is-",
        options: [
            "False",
            "Partly False",
            "True",
            "Partly True"
        ],
        a: 2 // True
    },
    {
        q: "One of these cannot be included under health behaviors. Identify.",
        options: [
            "Balanced diet",
            "Regular Exercise",
            "Regular Healthcare",
            "Unprotected Sex"
        ],
        a: 3 // Unprotected Sex
    },
    {
        q: "Epidemiological methods are more likely to be used by-",
        options: [
            "Social Psychologists",
            "Health Psychologists",
            "Industrial Psychologists",
            "Educational Psychologists"
        ],
        a: 1 // Health Psychologists
    },
    {
        q: "The first official Health Psychology journal was published in the year____",
        options: [
            "1989",
            "1998",
            "1982",
            "1984"
        ],
        a: 2 // 1982
    },
    {
        q: "Health psychology has been recognized within the American Psychological Association as-",
        options: [
            "Division 36",
            "Division 43",
            "Division 38",
            "Division 48"
        ],
        a: 2 // Division 38
    },
    {
        q: "One of these is currently not an area of activity for health psychology-",
        options: [
            "Promotion of health",
            "Prescription of medicines",
            "Prevention of illness",
            "Treatment of illness"
        ],
        a: 1 // Prescription of medicines
    },
    {
        q: "Acceptance of the clinician’s advice, and engaging in recommended health behavior is an example of-",
        options: [
            "Diagnosis",
            "Psychotherapy",
            "Consultation",
            "Compliance"
        ],
        a: 3 // Compliance
    },
    {
        q: "Psychological processes can complicate diagnostic procedures. This statement is-",
        options: [
            "True",
            "Partly True",
            "False",
            "Partly False"
        ],
        a: 0 // True
    },
    {
        q: "In the development of physical disease symptoms, Sigmund Freud emphasized on the role of –",
        options: [
            "Unconscious psychological factors",
            "Unconscious physical factors",
            "Conscious psychological factors",
            "Conscious physical factors"
        ],
        a: 0 // Unconscious psychological factors
    },
    {
        q: "Emotion is accompanied by physiological changes — This finding was put forward by",
        options: [
            "Dunbar",
            "Freud",
            "Cannon",
            "Franz"
        ],
        a: 2 // Cannon
    },
    {
        q: "The link between personal conflicts and specific diseases as a cause of psychosomatic disorders were proposed by-",
        options: [
            "Helen Flanders Dunbar",
            "Sigmund Freud",
            "Walter Cannon",
            "Franz Alexander"
        ],
        a: 3 // Franz Alexander
    },
    {
        q: "Physicians supporting the psychosomatic approach call for an acceptance of the-",
        options: [
            "Biomedical model",
            "Biopsychosocial model",
            "Psychosocial model",
            "Biosocial model"
        ],
        a: 1 // Biopsychosocial model
    },
    {
        q: "Research done at one time is called ________________",
        options: [
            "Replication",
            "Correlational",
            "Cross-sectional",
            "Longitudinal"
        ],
        a: 2 // Cross-sectional
    },
    {
        q: "A psychologist using the method of naturalistic observation would",
        options: [
            "Carefully design controlled situations in which to observe behavior.",
            "Rely on observations of subjects' responses to questionnaires.",
            "Observe behaviour as it happens outside the laboratory or clinic",
            "Make records of the behavior of clients treated in therapy."
        ],
        a: 2 // Observe behaviour as it happens outside the laboratory or clinic
    },
    {
        q: "The most powerful research tool is a (an)",
        options: [
            "Clinical study.",
            "Experiment.",
            "Survey.",
            "Correlational study."
        ],
        a: 1 // Experiment
    },
    {
        q: "An advantage of the experimental method in psychology is ___",
        options: [
            "The identification of a cause- and-effect relationship.",
            "Similar to the correlational method in that causality is determined.",
            "That the surroundings are always similar to real life experiences.",
            "That it is an informal way to investigate behavior."
        ],
        a: 0 // The identification of a cause-and-effect relationship
    },
    {
        q: "The new definition of health recognizes the links between the",
        options: [
            "Individual and her/his environment",
            "Parents and their children’s health",
            "Education level and tolerance",
            "Individual and his/her mentalities"
        ],
        a: 0 // Individual and her/his environment
    },
    {
        q: "The epidemiological triangle concept is based on the theory ____",
        options: [
            "Doctrine of Specific Etiology",
            "Theory of epidemiology",
            "Doctrine of the fittest",
            "Theory of triangle health"
        ],
        a: 0 // Doctrine of Specific Etiology
    },
    {
        q: "According to Doctrine of Specific Etiology each disease has ____",
        options: [
            "Multiple determinants",
            "Single and specific cause",
            "Myriad causes",
            "Unique cause"
        ],
        a: 1 // Single and specific cause
    },
    {
        q: "The epidemiological triangle portrays the interaction between__",
        options: [
            "Agent, host and environment",
            "Individual, host and environment",
            "Individual, disease and environment",
            "Agent, individual and host"
        ],
        a: 0 // Agent, host and environment
    },
    {
        q: "Lalonde highlights four categories or determinants of health which, he argues, any health problem can be",
        options: [
            "Determined",
            "Assessed",
            "Traced back",
            "Relied upon"
        ],
        a: 2 // Traced back
    },
    {
        q: "Which is not one dimension of five illness representation domains identified by Leventhal?",
        options: [
            "Control/cure",
            "Timeline",
            "Consequences",
            "Transmissibility"
        ],
        a: 3 // Transmissibility
    },
    {
        q: "This refers to the patient’s perceptions of the possible effects of the illness on their life.",
        options: [
            "Consequences",
            "Control/cure",
            "Timeline",
            "Identity"
        ],
        a: 0 // Consequences
    },
    {
        q: "What are the three stages in our response to illness?",
        options: [
            "Perceiving symptoms, active checking for additional symptoms, seeking help",
            "Perceiving symptoms, comparing symptoms with an illness prototype, taking action",
            "Perceiving symptoms, seeking medical support, engaging in treatment",
            "Perceiving symptoms, interpreting symptoms as illness, planning and taking action"
        ],
        a: 3 // Perceiving symptoms, interpreting symptoms as illness, planning and taking action
    },
    {
        q: "The existence of feedback loops contribute to Leventhal's model being referred to as ______.",
        options: [
            "Self-monitoring",
            "Self-regulatory",
            "Self-reporting",
            "Self-perceiving"
        ],
        a: 1 // Self-regulatory
    },
    {
        q: "Medical students' disease is best described by which of the following?",
        options: [
            "A disease that only occurs to medical students because they are around sick people all day.",
            "Can occur in anyone who studies about illnesses because all that studying makes everyone ill",
            "Believing that you might have the symptoms of each disease that one studies",
            "It is not contagious"
        ],
        a: 2 // Believing that you might have the symptoms of each disease that one studies
    },
    {
        q: "Which of the following statements is true regarding Seriousness of the Symptoms?",
        options: [
            "Symptoms that affect less valued parts of the body are usually interpreted as more Serious.",
            "Symptoms that affect highly valued parts of the body are usually interpreted as more Serious",
            "Symptoms that affect highly valued parts of the body are usually interpreted as less Serious",
            "Symptoms that affect less valued parts of the body are usually interpreted as less Serious"
        ],
        a: 3 // Symptoms that affect less valued parts of the body are usually interpreted as less Serious
    },
    {
        q: "Which of the following terms best describes an individual who express distress and conflict through bodily symptoms?",
        options: [
            "Worried well",
            "Somaticizers",
            "Natural healers",
            "Pessimist"
        ],
        a: 1 // Somaticizers
    },
    {
        q: "Cultural differences exist in the use of health system. Which of the following is an accurate statement?",
        options: [
            "Minority groups use cancer screening services such as having a mammogram, and prostate screening more often.",
            "Members of ethnic minorities were more likely to have consulted a physician in the previous year than Caucasians.",
            "Aboriginals were less likely to have consulted a physician in the previous year than Caucasians.",
            "Minority groups use cancer screening services such as having a mammogram, or prostate screening less often."
        ],
        a: 3 // Minority groups use less often
    },
    {
        q: "Which of the following is Not a dimension of Illness cognition?",
        options: [
            "Identity",
            "Consequences",
            "Timeline",
            "Hereditary"
        ],
        a: 3 // Hereditary
    },
    {
        q: "Which of the following is Not an example of Approach coping?",
        options: [
            "Taking medicines",
            "Visiting doctor",
            "Denial of illness",
            "Resting"
        ],
        a: 2 // Denial of illness
    },
    {
        q: "What are the two channels through which an individual may be confronted with the problem of a potential illness?",
        options: [
            "Symptom perception & family member’s perception",
            "Symptom perception & Social messages",
            "Family member’s perception & doctor’s opinion",
            "Social messages & other patients’ perception with the illness"
        ],
        a: 1 // Symptom perception & Social messages
    },
    {
        q: "An individual evaluating the effectiveness of the coping strategy and determining whether to continue with this strategy or whether to opt for an alternative one is termed as ______",
        options: [
            "Appraisal",
            "Interpretation",
            "Perceived control",
            "Resilience"
        ],
        a: 0 // Appraisal
    },
    {
        q: "Which of the following statement is False regarding Self-Regulatory model of illness cognition?",
        options: [
            "This model is developed by Leventhal.",
            "It suggests that illness is dealt by individuals in the same way as other problems.",
            "This model has three stages.",
            "Its major assumption is that the role individual with illness is passive."
        ],
        a: 3 // False statement
    },
    {
        q: "The _______ defines the links between beliefs, attitudes, norms, intentions, and behaviors of individuals.",
        options: [
            "Theory of Reasoned Action",
            "Health belief model",
            "Theory of social action",
            "The theory of planned behavior"
        ],
        a: 0 // Theory of Reasoned Action
    },
    {
        q: "A model that attempts to integrate individual psychological processes with social contextual factors is _____",
        options: [
            "Health belief model",
            "Theory of social action",
            "The theory of planned behavior",
            "The Stages-of-Change Model"
        ],
        a: 1 // Theory of social action
    },
    {
        q: "The same message coming from two different sources can elicit different reactions is one out of four elements in peripheral routes to persuasion. Identify the component",
        options: [
            "The communicator",
            "The message",
            "How the message is communicated",
            "The audience"
        ],
        a: 0 // The communicator
    },
    {
        q: "An individual who is considering change related to their health behaviour is at which stage according to Transtheoretical model?",
        options: [
            "Pre-Contemplation",
            "Contemplation",
            "Preparation",
            "Termination"
        ],
        a: 1 // Contemplation
    },
    {
        q: "Self-regulation is a concept that derives from _____________",
        options: [
            "Theory of planned behavior",
            "Health belief model",
            "Theory of Reasoned Action",
            "Cognitive social-learning theory"
        ],
        a: 3 // Cognitive social-learning theory
    },
    {
        q: "Brownell et al., 1986 stated that in order to really succeed at long-term maintenance and control, an individual must become aware of his/her own potentially ______ and learn to use self-instructions that are compatible with success.",
        options: [
            "Encouraging positive thoughts",
            "Damaging negative self-statements",
            "Supportive member of the family or friends",
            "Destructive behaviors"
        ],
        a: 1 // Damaging negative self-statements
    },
    {
        q: "Operant behavior that is rewarded or reinforced by the individual or by the environment is more likely to be _________________",
        options: [
            "Relapse",
            "Repeat",
            "Reduce gradually",
            "Fluctuate"
        ],
        a: 1 // Repeat
    },
    {
        q: "Identify which of the following is not amongst five factors of influence for health-related behaviors and beliefs?",
        options: [
            "Intrapersonal",
            "Interpersonal",
            "Community",
            "Cross-cultural"
        ],
        a: 3 // Cross-cultural
    },
    {
        q: "What is the third stage of self-regulation process?",
        options: [
            "Self-monitoring",
            "Self-observation",
            "Self-evaluation",
            "Self-reinforcement"
        ],
        a: 3 // Self-reinforcement
    },
    {
        q: "“Between the Seams,” for instance, is a play performed by adolescents and young adults at schools and community centers around the ________ to spread awareness about HIV prevention, while emphasizing tolerance and understanding of the illness.",
        options: [
            "Canada",
            "United States",
            "Australia",
            "India"
        ],
        a: 1 // United States
    },
    {
        q: "The five core components representing the key strategies that contribute to success in building healthy school communities was developed by",
        options: [
            "The Propel Centre for Population Health Impact in 2012",
            "World Health Organization",
            "Canadian Health Care System",
            "National Society for Children and Adults with Autism"
        ],
        a: 1 // World Health Organization
    },
    {
        q: "Patients of physicians who communicate well have a ____% higher rate of adherence",
        options: [
            "30",
            "29",
            "19",
            "17"
        ],
        a: 2 // 19%
    },
    {
        q: "Systematic investigation of the merit, worth and significance of an organized health promotion action is done at which step of Workplace Health Model?",
        options: [
            "Assessment",
            "Planning",
            "Implementation",
            "Evaluation"
        ],
        a: 3 // Evaluation
    },
    {
        q: "_____ gave rise to the Temperance Movement in the eighteenth and nineteenth century, which encouraged total abstinence from alcohol.",
        options: [
            "Disease models",
            "Moral models",
            "Health models",
            "Diathesis models"
        ],
        a: 1 // Moral models
    },
    {
        q: "Individuals who begin drinking at _____ age are at a higher risk of problem drinking or physical dependence on alcohol.",
        options: [
            "A later",
            "An early",
            "A middle",
            "A late"
        ],
        a: 1 // An early
    },
    {
        q: "Body mass index is calculated by dividing your ______.",
        options: [
            "Weight in kilogram and height in meter",
            "weight in kilograms by your height in meters squared",
            "weight in kilograms and height in meters’ cube",
            "Both weight and height in meters squared"
        ],
        a: 1 // weight (kg) / height^2 (m²)
    },
    {
        q: "The risk of alcoholism is _____ for people who have a parent or other close relatives who have problems with alcohol.",
        options: [
            "Higher",
            "Lower",
            "Moderate",
            "Unpredictable"
        ],
        a: 0 // Higher
    },
    {
        q: "Which of the following is Not an alternative medicine for alcohol abuse?",
        options: [
            "Yoga",
            "Meditation",
            "Acupuncture",
            "Injected medication"
        ],
        a: 3 // Injected medication
    },
    {
        q: "He is the psychologist who coined the term ‘eustress’. He is-",
        options: [
            "Hans Selye",
            "Susan Folkman",
            "Richard Lazaraus",
            "Walter Cannon"
        ],
        a: 0 // Hans Selye
    },
    {
        q: "The ‘fight-or-flight response’ in humans was first identified by-",
        options: [
            "Kamarck & Mermelstein",
            "Richard Lazarus",
            "Hans Selye",
            "Walter Cannon"
        ],
        a: 3 // Walter Cannon
    },
    {
        q: "Selye’s General Adaptation Syndrome is known as a-",
        options: [
            "Psychological Model of Stress",
            "Biological Model of Stress",
            "Interactional Model of Stress",
            "Transactional Model of Stress"
        ],
        a: 1 // Biological Model of Stress
    },
    {
        q: "Walter Cannon describes stress in terms of a/an-",
        options: [
            "Stimulus",
            "Response",
            "Interaction",
            "Interference"
        ],
        a: 1 // Response
    },
    {
        q: "The activation of this system is responsible for restoring physiological balance after a stress response-",
        options: [
            "The Autonomic Nervous System",
            "The Sympathetic Nervous System",
            "The Parasympathetic Nervous System",
            "The Peripheral Nervous System"
        ],
        a: 2 // Parasympathetic Nervous System
    },
    {
        q: "Galvanic skin response is a –",
        options: [
            "Biochemical Measure of stress",
            "Psychological Measure of stress",
            "Physiological Measure of stress",
            "Psychosocial Measure of stress"
        ],
        a: 2 // Physiological Measure of stress
    },
    {
        q: "The level of secretion of catecholamine’s is a –",
        options: [
            "Psychological Measure of stress",
            "Physiological Measure of stress",
            "Biochemical Measure of stress",
            "Psychosocial Measure of stress"
        ],
        a: 2 // Biochemical Measure of stress
    },
    {
        q: "The Social Readjustment Rating Scale (SRRS) was developed by",
        options: [
            "Holmes and Rahe",
            "Lazarus and Folkman",
            "Cohen and",
            "Delongis and Folkman"
        ],
        a: 0 // Holmes and Rahe
    },
    {
        q: "The Everyday Hassles Scale was developed by-",
        options: [
            "Thomas Holmes",
            "Susan Folkman",
            "Anita Delongis",
            "Richard Lazarus"
        ],
        a: 3 // Richard Lazarus
    },
    {
        q: "Adrenocorticotropic hormone (ACTH) is a hormone secreted by the-",
        options: [
            "Thymus gland",
            "Adrenal gland",
            "Thyroid gland",
            "Pituitary gland"
        ],
        a: 3 // Pituitary gland
    },
    {
        q: "Glucocorticoids and mineralocorticoids are hormones secreted by the-",
        options: [
            "Adrenal medulla",
            "Anterior pituitary",
            "Adrenal cortex",
            "Posterior pituitary"
        ],
        a: 2 // Adrenal cortex
    },
    {
        q: "Vasopressin and oxytocin are hormones secreted by the –",
        options: [
            "Posterior pituitary gland",
            "Anterior pituitary gland",
            "Pineal gland",
            "Thymus gland"
        ],
        a: 0 // Posterior pituitary gland
    },
    {
        q: "The HPA axis involves an activation of –",
        options: [
            "The hypothalamus, Pituitary gland and the adrenal cortex",
            "The hypothalamus, Pituitary gland and the adrenal medulla",
            "The hypothalamus, Pituitary gland and the sympathetic nervous system",
            "The hypothalamus, Pineal gland and the adrenal cortex"
        ],
        a: 0 // Hypothalamus + Pituitary + Adrenal cortex
    },
    {
        q: "Peptic ulcer is an ulceration which involves the-",
        options: [
            "Respiratory tract",
            "Gastrointestinal tract",
            "Urinary tract",
            "Alimentary tract"
        ],
        a: 1 // Gastrointestinal tract
    },
    {
        q: "The influence of chronic stress on gastric functioning causes-",
        options: [
            "Reduced production of gastric acid",
            "Increased production of gastric acid",
            "Increased sympathetic activity",
            "Increased production of cortisol"
        ],
        a: 1 // Increased production of gastric acid
    },
    {
        q: "Seasonal pollens and animal hairs are examples of",
        options: [
            "Specific allergens",
            "Irritants",
            "Nonspecific allergens",
            "Annoyances"
        ],
        a: 0 // Specific allergens
    },
    {
        q: "Psychosocial stress contributes to sports injuries by",
        options: [
            "Reducing concentration",
            "Increasing muscle tension",
            "Increasing vigilance",
            "Both a and b"
        ],
        a: 3 // Both a and b
    },
    {
        q: "Peptic ulcers are more common in",
        options: [
            "Men",
            "Women",
            "Children",
            "Elderly"
        ],
        a: 0 // Men
    },
    {
        q: "Coping is a–",
        options: [
            "Dynamic process",
            "Static process",
            "Complimentary process",
            "Supplementary process"
        ],
        a: 0 // Dynamic process
    },
    {
        q: "Successful coping does not involve one of the following",
        options: [
            "Realistic view of the problem",
            "Maintenance of emotional equilibrium",
            "Suppression of emotions",
            "Maintenance of satisfying relationships"
        ],
        a: 2 // Suppression of emotions
    },
    {
        q: "Residual coping occurs",
        options: [
            "Long before the stressful event",
            "Long after the stressful event",
            "Just before the stressful event",
            "Just after the stressful event"
        ],
        a: 1 // Long after the stressful event
    },
    {
        q: "A student tries to relax just before the start of the examination. This is an example of",
        options: [
            "Anticipatory coping",
            "Preventive coping",
            "Reactive coping",
            "Residual coping"
        ],
        a: 0 // Anticipatory coping
    },
    {
        q: "While stuck in a lift during a power-cut, Anil gave his packet of biscuits to the elderly person with him, though he himself was hungry. Anil was using",
        options: [
            "Self-focused goals of coping",
            "Compassion-focused goals of coping",
            "Other-focused goals of coping",
            "Generosity-focused goals of coping"
        ],
        a: 2 // Other-focused goals of coping
    },
    {
        q: "Human beings have an inherent need for control. This statement is –",
        options: [
            "True",
            "False",
            "Partly true",
            "Partly false"
        ],
        a: 0 // True
    },
    {
        q: "The theory of cognitive adaptation to stressful events was proposed by –",
        options: [
            "Taylor",
            "Higgins",
            "Lazarus",
            "Folkman"
        ],
        a: 0 // Taylor
    },
    {
        q: "Psychologists popularly refer to the cognitive techniques of gaining control as –",
        options: [
            "Defensive control",
            "Thought suppression",
            "Mental control",
            "Mood control"
        ],
        a: 2 // Mental control
    },
    {
        q: "The locus of control theory was put forward by –",
        options: [
            "Julian Rotter",
            "Carl Jung",
            "Richard Lazarus",
            "Suzanne Kobasa"
        ],
        a: 0 // Julian Rotter
    },
    {
        q: "The Big Five model of personality was developed by –",
        options: [
            "Gordon Allport",
            "Raymond Cattell",
            "Costa and McCrae",
            "Goldberg and Jung"
        ],
        a: 2 // Costa and McCrae
    },
    {
        q: "One of these is not a part of the Big Five model of personality",
        options: [
            "Neuroticism",
            "Psychoticism",
            "Conscientiousness",
            "Extraversion"
        ],
        a: 1 // Psychoticism
    },
    {
        q: "Openness to experience does not involve one of the following –",
        options: [
            "Non-conformity",
            "Ingenuity",
            "Insecurity",
            "Enterprise"
        ],
        a: 2 // Insecurity
    },
    {
        q: "Neurotics are most likely to engage in –",
        options: [
            "Problem-focused coping strategies",
            "Emotion-focused coping strategies",
            "Avoidant coping strategies",
            "Both b and c"
        ],
        a: 3 // Both b and c
    },
    // Week 6 - Social Support
    {
        q: "Your friend runs errands, buys groceries and cooks for you when you are stressed and busy looking after your ill brother. Your friend is providing you-",
        options: [
            "Emotional Social Support",
            "Tangible Social Support",
            "Informational Social Support",
            "Procedural Social Support"
        ],
        a: 1 // Tangible Social Support
    },
    {
        q: "This form of social support is most important in the case of chronic and prolonged stress-",
        options: [
            "Emotional Social Support",
            "Informational Social Support",
            "Tangible Social Support",
            "Procedural Social Support"
        ],
        a: 0 // Emotional Social Support
    },
    {
        q: "This form of social support is most important for alleviating geriatric depression-",
        options: [
            "Informational Social Support",
            "Tangible Social Support",
            "Procedural Social Support",
            "Emotional Social Support"
        ],
        a: 3 // Emotional Social Support
    },
    {
        q: "Existence of a good social support system can lead to a-",
        options: [
            "Maintenance of negative health practices",
            "Neglect of positive health behaviours",
            "Maintenance of positive health practices",
            "Adoption of negative health behaviours"
        ],
        a: 2 // Maintenance of positive health practices
    },
    {
        q: "Functional measurements of social support analyze-",
        options: [
            "The number of social contacts",
            "The interlinkages between social networks",
            "The interdependence of social contacts",
            "The tasks undertaken by social networks"
        ],
        a: 3 // The tasks undertaken by social networks
    },
    // Week 6 - Stress Management – Relaxation, Meditation, Biofeedback, Physical Activity and Stress Inoculation Training
    {
        q: "The Transactional model of stress put forward by _______",
        options: [
            "Richard Lazarus and Susan Folkman",
            "Herbert Benson",
            "Edmond Jacobson",
            "Joseph Kamiya and Barbara Brown"
        ],
        a: 0 // Richard Lazarus and Susan Folkman
    },
    {
        q: "Which of the following is Not one of the three stages of stress inoculation?",
        options: [
            "Conceptualization",
            "Skill Acquisition and Rehearsal",
            "Relaxation",
            "Application and Follow-through"
        ],
        a: 2 // Relaxation
    },
    {
        q: "Which of the following technique is based on the assumption that a person cannot concentrate on more than one thing at a time and concentration on a delightful or pleasant scene or image which is powerful enough can help in averting attention from the painful experience?",
        options: [
            "Guided imagery",
            "Progressive muscle relaxation technique",
            "Stress inoculation",
            "Mindfulness meditation"
        ],
        a: 0 // Guided imagery
    },
    {
        q: "________ helps an individual gains information about changes in biological responses as they are taking place.",
        options: [
            "Mindfulness meditation",
            "Stress inoculation technique",
            "Progressive muscle relaxation technique",
            "Biofeedback"
        ],
        a: 3 // Biofeedback
    },
    {
        q: "Which model focuses solely on the person’s thought processes that determine his perception of and response to a stressful event?",
        options: [
            "Transactional model",
            "Behavioural theory",
            "Cognitive theory",
            "Innate health model"
        ],
        a: 3 // Innate health model
    },
    // Week 6 - Nature and Socio-cultural Context of Illness
    {
        q: "According to the WHO, health is –",
        options: [
            "An absence of disease",
            "A state of complete physical well-being",
            "A state of physical, mental and social well-being",
            "A state of physical and mental well-being"
        ],
        a: 2 // A state of physical, mental and social well-being
    },
    {
        q: "Edward Suchman proposed –",
        options: [
            "Five stages of illness",
            "Seven stages of illness",
            "Six stages of illness",
            "Four stages of illness"
        ],
        a: 0 // Five stages of illness
    },
    {
        q: "In experiments by Zborowski, Italian Americans were found to –",
        options: [
            "Exaggerate pain experiences",
            "Minimize pain experiences",
            "Negate pain experiences",
            "Report pain experiences"
        ],
        a: 0 // Exaggerate pain experiences
    },
    {
        q: "‘Apply X cream to protect your face from dust and pollutants’. This message is more likely to be accepted by people from –",
        options: [
            "Individualistic cultures",
            "Collectivist cultures",
            "Developed cultures",
            "Primitive cultures"
        ],
        a: 1 // Collectivist cultures
    },
    // Week 6 - Perceiving Symptoms – Physiological and Psychological symptoms
    {
        q: "The meanings that we give to bodily symptoms are based on-",
        options: [
            "Medical diagnosis",
            "Past experiences",
            "Available medications",
            "Both a and b"
        ],
        a: 3 // Both a and b
    },
    {
        q: "A desire to eat unusual things such as dirt or hair may be a symptom of –",
        options: [
            "Deficiency",
            "Emotional distress",
            "Both a and b",
            "Hunger"
        ],
        a: 2 // Both a and b
    },
    {
        q: "Insomnia is characterized by –",
        options: [
            "A lack of sleep",
            "Excessive sleep",
            "Disturbed sleep",
            "Nightmares"
        ],
        a: 0 // A lack of sleep
    },
    {
        q: "The physical and psychological symptoms of disease may act as causative factors for each other. This statement is",
        options: [
            "True",
            "False",
            "Partly true",
            "Partly false"
        ],
        a: 0 // True
    },
    // Week 7 - Factors and Accuracy in Symptom Perception
    {
        q: "A chronic scrutiny of the body is –",
        options: [
            "Beneficial",
            "Abnormal",
            "Natural",
            "Essential"
        ],
        a: 1 // Abnormal
    },
    {
        q: "Neurotic individuals tend to –",
        options: [
            "Perceive more symptoms",
            "Perceive less symptoms",
            "Exaggerate minor symptoms",
            "Both a and c"
        ],
        a: 3 // Both a and c
    },
    {
        q: "People differ in the extent to which they can bear pain. This statement is –",
        options: [
            "True",
            "False",
            "Partly true",
            "Partly false"
        ],
        a: 0 // True
    },
    {
        q: "Rita who is suffering from diabetes is busy in the preparations for her sister’s wedding. She is –",
        options: [
            "More likely to notice the symptoms",
            "Less likely to notice the symptoms",
            "More likely to exaggerate the symptoms",
            "Both a and c"
        ],
        a: 1 // Less likely to notice the symptoms
    },
    {
        q: "Type A individuals are more likely to report symptoms when –",
        options: [
            "Engaged in a task",
            "Not engaged in a task",
            "Just before a task",
            "Regulating a task"
        ],
        a: 1 // Not engaged in a task
    },
    // Week 7 - Responding to Illness – Types of Illness Response, Illness Behaviour as Coping, Delaying Medical Care and Overusing Medical Care
    {
        q: "Illness affirmation is –",
        options: [
            "Normal response to illness",
            "Abnormal response to illness",
            "Routine response to illness",
            "Essential response to illness"
        ],
        a: 1 // Abnormal response to illness
    },
    {
        q: "One of these is not a privilege associated with the sick-role –",
        options: [
            "An exemption from daily chores",
            "An exemption from job responsibilities",
            "An opportunity for self-medication",
            "An opportunity for seeking assistance"
        ],
        a: 2 // An opportunity for self-medication
    },
    {
        q: "The delay between the making of an appointment with the doctor and actually receiving medical care is –",
        options: [
            "Illness delay",
            "Medical delay",
            "Utilization delay",
            "Appraisal delay"
        ],
        a: 1 // Medical delay
    },
    {
        q: "The delay between the appraisal of symptoms and the receiving of medical care is referred to as –",
        options: [
            "Total delay",
            "Medical delay",
            "Utilization delay",
            "Appraisal delay"
        ],
        a: 0 // Total delay
    },
    {
        q: "One of these is not a cause of delay in medical care –",
        options: [
            "Expense of the treatment",
            "Lack of concern",
            "Fear of treatment",
            "Physician self-referral"
        ],
        a: 3 // Physician self-referral
    },
    // Week 7 - Psycho social Dimensions of Chronic Illness and Disability- Psychological Aspects, Social Aspects and Helping People Cope
    {
        q: "A functional limitation that interferes with a person’s ability to walk, lift, hear or learn is called...",
        options: [
            "Handicap",
            "Disability",
            "Impairment",
            "Life limiting illness"
        ],
        a: 1 // Disability
    },
    {
        q: "A condition that interferes with daily functioning for more than 3 months in a year, causes hospitalization of more than 1 month in a year, or is likely to do either of these is termed as:",
        options: [
            "Chronic Illness",
            "Disability",
            "Lifetime Impairment",
            "Handicap"
        ],
        a: 0 // Chronic Illness
    },
    {
        q: "What is true of chronic conditions?",
        options: [
            "Chronic conditions are only found in the lower socioeconomic strata",
            "Chronic conditions are only found in the elderly",
            "Chronic conditions occur in every race and culture",
            "Chronic conditions are decreasing"
        ],
        a: 2 // Chronic conditions occur in every race and culture
    },
    {
        q: "Which of the following would be a characteristic of a chronic illness?",
        options: [
            "Slow, progressive decline",
            "Requires short term surveillance",
            "Reversible pathology",
            "Cure likely"
        ],
        a: 0 // Slow, progressive decline
    },
    // Week 7 - Breaking the Bad News: Helping the Patient Understand the situation
    {
        q: "Bad news according to researchers, is always in the eye of the –",
        options: [
            "Patient",
            "Relatives",
            "Physician",
            "Siblings"
        ],
        a: 0 // Patient
    },
    {
        q: "During the 1950s, 1960s and 1970s physicians felt that breaking the bad news especially of terminal illness can be-",
        options: [
            "Inhuman",
            "Conducive",
            "Disastrous",
            "Both a and c"
        ],
        a: 3 // Both a and c
    },
    {
        q: "The outcomes of conveying the bad news to the patient depend on-",
        options: [
            "One variable",
            "Two variables",
            "Multiple variables",
            "Three variables"
        ],
        a: 2 // Multiple variables
    },
    {
        q: "During the breaking of the bad news the time pressure affects-",
        options: [
            "The Patient",
            "The Physician",
            "The Family Members",
            "All the above"
        ],
        a: 3 // All the above
    },
    // Week 8 - Psychological Intervention and Chronic Illness
    {
        q: "Pharmacological Interventions are more likely to be used in cases of –",
        options: [
            "Depression",
            "Anxiety",
            "Lack of Confidence",
            "Both a and b"
        ],
        a: 3 // Both a and b
    },
    {
        q: "Individual therapy in chronically ill patients is likely to be-",
        options: [
            "Episodic",
            "Continuous",
            "Relevant",
            "Irrelevant"
        ],
        a: 0 // Episodic
    },
    {
        q: "Individual therapy with chronically ill patients requires co-ordination with-",
        options: [
            "Physician",
            "Patient’s Family",
            "Both a and b",
            "Neither of the above"
        ],
        a: 2 // Both a and b
    },
    {
        q: "In case of Individual therapy with chronically ill patients the patients’ defenses are more likely to be-",
        options: [
            "Challenged",
            "Respected",
            "Denied",
            "Contradicted"
        ],
        a: 1 // Respected
    },
    {
        q: "Support groups may be initiated by-",
        options: [
            "The Therapist",
            "The Patient",
            "Either a or b",
            "Neither a or b"
        ],
        a: 2 // Either a or b
    },
    // Week 8 - Chronic Illness and Rehabilitation
    {
        q: "The most critical aspect of rehabilitation in chronic illness is-",
        options: [
            "Patient co-management",
            "Family co-management",
            "Physician co-management",
            "Psychologist co-management"
        ],
        a: 0 // Patient co-management
    },
    {
        q: "Stress is not major cause in the recurrence and aggravation of several diseases. This statement is-",
        options: [
            "True",
            "False",
            "Partly True",
            "Partly False"
        ],
        a: 1 // False
    },
    {
        q: "Care giving can also strain the relationship between-",
        options: [
            "The patient and the caregiver",
            "The caregiver and his or her family",
            "The caregiver and friends",
            "All of the above"
        ],
        a: 3 // All of the above
    },
    {
        q: "Many chronically ill patients such as heart patients, cancer patients and AIDS patients may face job discrimination. This statement is-",
        options: [
            "True",
            "False",
            "Partly True",
            "Partly False"
        ],
        a: 0 // True
    },
    {
        q: "Adherence among chronically ill patients is often-",
        options: [
            "Extremely High",
            "Very Regular",
            "Very Poor",
            "Both a and b"
        ],
        a: 2 // Very Poor
    },
    // Week 8 - Illness and Gender Quotient
    {
        q: "Sex refers to-",
        options: [
            "A Political construct",
            "A Biological construct",
            "A Social Construct",
            "A Cultural Construct"
        ],
        a: 0 // A Political construct
    },
    {
        q: "Which of these statements do not apply to gender? Identify",
        options: [
            "It is a social construct",
            "It is a relative concept",
            "It is a biological construct",
            "It is a dynamic concept"
        ],
        a: 2 // It is a biological construct
    },
    {
        q: "Health and Illness can be understood solely in terms of biological factors. This statement is-",
        options: [
            "True",
            "False",
            "Partly True",
            "Partly False"
        ],
        a: 1 // False
    },
    {
        q: "Women particularly in developing countries are found to be engaged in-",
        options: [
            "Productive labour",
            "Reproductive labour",
            "Domestic labour",
            "Both b and c"
        ],
        a: 3 // Both b and c
    },
    // Week 8 - Illness in Childhood
    {
        q: "Chronic illnesses are-",
        options: [
            "Long term",
            "Permanent",
            "Short term",
            "Both a and b"
        ],
        a: 3 // Both a and b
    },
    {
        q: "A child suffering from chronic illness can be-",
        options: [
            "Continuously ill",
            "Intermittently Ill without cure",
            "Either a or b",
            "Necessarily continuously ill"
        ],
        a: 2 // Either a or b
    },
    {
        q: "One of these is not an example of a chronic childhood illness. Identify",
        options: [
            "Cancer",
            "Common Cold",
            "Asthma",
            "Spina bifida"
        ],
        a: 1 // Common Cold
    },
    {
        q: "Hospitalization and separation from family due to chronic illness is most likely to affect-",
        options: [
            "Babies and toddlers",
            "Adolescents",
            "Adults",
            "Parents"
        ],
        a: 0 // Babies and toddlers
    },
    {
        q: "Children express their feelings only verbally. This statement is-",
        options: [
            "True",
            "False",
            "Partly True",
            "Partly False"
        ],
        a: 1 // False
    },
    // Week 9 - Illness and Ageing
    {
        q: "Fractures become more common in old age due to-",
        options: [
            "Loss in bone density",
            "Increase in bone size",
            "Increase in Muscle mass",
            "Both b and c"
        ],
        a: 0 // Loss in bone density
    },
    {
        q: "During old age blood cells that fight infections or white blood cells become-",
        options: [
            "More effective",
            "Increase in number",
            "Less effective",
            "More dominant"
        ],
        a: 2 // Less effective
    },
    {
        q: "One of these does not constitute a healthy lifestyle change for the elderly. Identify.",
        options: [
            "Smoking cessation",
            "Taking over the counter drugs",
            "Regular doctor visits",
            "Reduced alcohol intake"
        ],
        a: 1 // Taking over the counter drugs
    },
    {
        q: "Safety measures that can be used by the elderly include:",
        options: [
            "Canes and Walkers",
            "Wheelchairs and Crutches",
            "Shower Seats",
            "All of the above"
        ],
        a: 3 // All of the above
    },
    {
        q: "Measures to help caregivers cope with illness include-",
        options: [
            "Accepting Feelings",
            "Asking family and friends for help",
            "Seeking professional and community support",
            "All of the above"
        ],
        a: 0 // Accepting Feelings
    },
    // Week 9 - Living with Diabetes- Nature, Psychosocial Dimensions and Helping People Live with Diabetes
    {
        q: "Nurse should recognize which symptom as a chief signal of diabetes mellitus?",
        options: [
            "Nausea",
            "Seizure",
            "Hyperactivity",
            "Frequent Urination"
        ],
        a: 3 // Frequent Urination
    },
    {
        q: "Why is blood glucose monitoring needed in people suffering from diabetes mellitus?",
        options: [
            "Easier method of testing",
            "Less expensive",
            "Better management of diabetes",
            "Greater sense of control over diabetes"
        ],
        a: 2 // Better management of diabetes
    },
    {
        q: "Effective collaborative management of diabetes includes:",
        options: [
            "Using insulin with all patients to achieve glycemic goals",
            "Relying on the health care provider as the central figure in the program for good control",
            "Relying solely on nutritional therapy as the initial treatment modality for all patients with diabetes",
            "Aiming for a balance of diet, activity and medications together with appropriate monitoring and patient and family teaching."
        ],
        a: 3 // Aiming for a balance of diet, activity and medications together with monitoring and teaching
    },
    {
        q: "The nurse assists the patient with nutritional therapy of diabetes with the knowledge that a “diabetic diet” is designed",
        options: [
            "To be used only for type 1 diabetes",
            "For use during periods of high stress",
            "To normalize blood glucose by elimination of sugar",
            "To help normalize blood glucose through a balanced diet"
        ],
        a: 3 // To help normalize blood glucose through a balanced diet
    },
    {
        q: "In teaching a newly diagnosed type 1 diabetic ‘survival skills’, the nurse includes information about:",
        options: [
            "Weight loss measures",
            "Elimination of sugar from diet",
            "Need to reduce physical activity",
            "Self-monitoring of blood glucose"
        ],
        a: 3 // Self-monitoring of blood glucose
    },
    // Week 9 - Dealing with Arthritis - Nature, Psychosocial Dimensions and Helping People Live with Arthritis
    {
        q: "What is the condition called: inflammation of the joints, lack of mobility but only until pain goes away?",
        options: [
            "Arthritis",
            "Diabetes mellitus",
            "Alzheimer’s disease",
            "Chronic illness"
        ],
        a: 0 // Arthritis
    },
    {
        q: "Arthritis does not affect this area of the body:",
        options: [
            "Small joints",
            "Knees",
            "Wrists",
            "Stomach"
        ],
        a: 3 // Stomach
    },
    {
        q: "In _______ the stiffness is worse after rest, such as the first thing in the morning, and often lasts at least 30 minutes or more.",
        options: [
            "Diabetes",
            "Rheumatoid Arthritis",
            "Alzheimer’s disease",
            "Chronic fatigue syndrome"
        ],
        a: 1 // Rheumatoid Arthritis
    },
    {
        q: "______ has the tendency to affect the larger weight bearing joints:",
        options: [
            "Diabetes Mellitus",
            "Rheumatoid Arthritis",
            "Osteoarthritis",
            "Chronic fatigue syndrome"
        ],
        a: 2 // Osteoarthritis
    },
    // Week 9 - Living with Alzheimer’s disease – Impact on Family, Helping Families Cope
    {
        q: "Neuronal loss in brain, chronic, progressive in nature and no cure. Identify the disease.",
        options: [
            "Diabetes Mellitus",
            "Cancer",
            "Alzheimer’s disease",
            "Arthritis"
        ],
        a: 2 // Alzheimer’s disease
    },
    {
        q: "What is the average duration of Alzheimer’s disease?",
        options: [
            "9 years",
            "8 years",
            "7 years",
            "6 years"
        ],
        a: 1 // 8 years
    },
    {
        q: "What is not right about Alzheimer’s disease?",
        options: [
            "Impairment of memory and attention",
            "Impairment of language and comprehension",
            "Good abstract thinking and judgement levels",
            "Depression"
        ],
        a: 2 // Good abstract thinking and judgement levels
    },
    {
        q: "Alzheimer’s is a type of dementia that causes problems with all these things except:",
        options: [
            "Memory",
            "Thinking",
            "Behavior",
            "Movement (at least not in initial stages)"
        ],
        a: 3 // Movement (at least not in initial stages)
    },
    // Week 10 - Cancer
    {
        q: "A group of diseases characterized by the uncontrolled and unregulated growth of cells is called",
        options: [
            "Hypertension",
            "Diabetes",
            "Cancer",
            "Headache"
        ],
        a: 2 // Cancer
    },
    {
        q: "Which of these is not a treatment option for Cancer?",
        options: [
            "Surgery",
            "Radiation",
            "Relaxation techniques",
            "Chemo therapy"
        ],
        a: 2 // Relaxation techniques
    },
    {
        q: "What is the malignant epithelial tumor called?",
        options: [
            "Carcinoma",
            "Melanoma",
            "Leukemia",
            "Glaucoma"
        ],
        a: 0 // Carcinoma
    },
    {
        q: "Which is not the most common cause of cancer deaths in men and women when compared to others?",
        options: [
            "Lungs",
            "Colon/rectum",
            "Prostate (men); Breast (women)",
            "Brain tumor"
        ],
        a: 3 // Brain tumor
    },
    {
        q: "A patient has primary lung cancer. Which of the following is the least likely carcinogen responsible?",
        options: [
            "Tobacco smoke",
            "Asbestos",
            "Radon gas",
            "UV radiation"
        ],
        a: 3 // UV radiation
    },
    // Week 10 - Coronary Heart Disease
    {
        q: "In common vocabulary, Myocardial infarction is nothing but:",
        options: [
            "Heart attack",
            "Asthma",
            "Cancer",
            "Chest pain"
        ],
        a: 0 // Heart attack
    },
    {
        q: "Angina Pectoris is a less serious but very painful form of coronary heart disease. What is it in common parlance?",
        options: [
            "Chest pain",
            "Cancer",
            "Asthma",
            "Heart attack"
        ],
        a: 0 // Chest pain
    },
    {
        q: "Clinicians and researchers have found that a few individuals are more prone to the coronary heart disease due to situational challenges they confront because of their personality make-up. These individuals are termed as:",
        options: [
            "Type A individuals",
            "Type B individuals",
            "Type C individuals",
            "Type D individuals"
        ],
        a: 0 // Type A individuals
    },
    {
        q: "These people are achievement oriented, competitive, perfectionists, time conscious and emotional. What are they called?",
        options: [
            "Type A individuals",
            "Type B individuals",
            "Type C individuals",
            "Type D individuals"
        ],
        a: 0 // Type A individuals
    },
    {
        q: "The Type D personality is very frequently termed to as the ____________ personality.",
        options: [
            "Disturbed",
            "Distressed",
            "Difficult",
            "Dangerous"
        ],
        a: 1 // Distressed
    },
    // Week 10 - Pain Physiology and Psychosocial Factors in the experience of Pain
    {
        q: "Pain is the perception of a noxious stimulus that begins in the dorsal horn and involves the entire",
        options: [
            "spinal cord and brain",
            "body and brain",
            "nerves and body",
            "tissues and bones"
        ],
        a: 0 // spinal cord and brain
    },
    {
        q: "One of the purposes of pain is that",
        options: [
            "Pain is painful",
            "Pain limits activity",
            "Pain is chronic",
            "Pain is acute"
        ],
        a: 1 // Pain limits activity
    },
    {
        q: "Pain can be described in terms of 3 components. They are",
        options: [
            "sensory, motivational, and cognitive",
            "sensory, psychological, and cognitive",
            "sensory, motivational, and perceptive",
            "sensory, perceptive, and cognitive"
        ],
        a: 0 // sensory, motivational, and cognitive
    },
    {
        q: "Acute pain does not",
        options: [
            "result in actual pain",
            "Lead to chronic pain",
            "last longer than six months",
            "need medicines"
        ],
        a: 2 // last longer than six months
    },
    // Week 10 - Theories of Pain – Early Pain Theories and Gate Control Theory
    {
        q: "According to which theory, untreated pain can result in death?",
        options: [
            "Specificity theory",
            "Pattern theory",
            "Pathophysiology of acute pain theory",
            "Gate Control Theory"
        ],
        a: 2 // Pathophysiology of acute pain theory
    },
    {
        q: "Which fibers create a hypothetical gate that can open and close?",
        options: [
            "Large fibers",
            "Small fibers",
            "Brain fibers",
            "Spinal cord fibers"
        ],
        a: 0 // Large fibers
    },
    {
        q: "Which theory proposed that there is no separate system for perceiving pain?",
        options: [
            "Specificity theory",
            "Pattern theory",
            "Pathophysiology of acute pain theory",
            "Gate Control Theory"
        ],
        a: 1 // Pattern theory
    },
    {
        q: "Which theory proposed that there is a separate system for perceiving pain?",
        options: [
            "Specificity theory",
            "Pattern theory",
            "Pathophysiology of acute pain theory",
            "Gate Control Theory"
        ],
        a: 0 // Specificity theory
    },
    // Week 11 - Controlling Pain – physical Methods, Psychological Approaches and Pain Clinics
    {
        q: "Pain intensity is measured by",
        options: [
            "Numerical rating scale",
            "Ordinal rating scale",
            "Using a scale",
            "Using thermometer"
        ],
        a: 0 // Numerical rating scale
    },
    {
        q: "Pain produced by a normally non-painful stimulus is called",
        options: [
            "Hyperalgesia",
            "Analgesia",
            "Apathy",
            "Allodynia"
        ],
        a: 3 // Allodynia
    },
    {
        q: "Increased response to a normally painful stimulus is",
        options: [
            "Hyperalgesia",
            "Analgesia",
            "Apathy",
            "Allodynia"
        ],
        a: 0 // Hyperalgesia
    },
    {
        q: "In which of the following methods, nerve transmission to an area is blocked through injection of an agent",
        options: [
            "Surgical lesions",
            "Neural Blockade",
            "Biofeedback",
            "Neuropathy"
        ],
        a: 1 // Neural Blockade
    },
    {
        q: "Neuro-destructive techniques sometimes worsen",
        options: [
            "Neuropathic pain",
            "Chronic pain",
            "Acute pain",
            "Mixed pain"
        ],
        a: 0 // Neuropathic pain
    },
    // Week 11 - Concept of Death and Facing the Thought of Death
    {
        q: "Five Powers to face the thought of death is put forth by",
        options: [
            "Buddhists",
            "Hindus",
            "Christians",
            "Jews"
        ],
        a: 0 // Buddhists
    },
    {
        q: "A determination of death must be made in accordance with",
        options: [
            "Standard religious practices",
            "Regular social norms",
            "Accepted medical standards",
            "One’s own concept of death"
        ],
        a: 2 // Accepted medical standards
    },
    {
        q: "The religion that believes there is rebirth after death is",
        options: [
            "Judaism",
            "Islam",
            "Christianity",
            "Hinduism"
        ],
        a: 3 // Hinduism
    },
    {
        q: "Research findings showed fear of death is highest during",
        options: [
            "Childhood",
            "Adolescence",
            "Adulthood",
            "Old age"
        ],
        a: 2 // Adulthood
    },
    {
        q: "Today more and more people are dying in",
        options: [
            "Homes",
            "Streets",
            "Bars",
            "Hospitals"
        ],
        a: 3 // Hospitals
    },
    // Week 11 - Dying Trajectories, Stages in Reactions to Dying, Social Relationships and Dying
    {
        q: "The concept of the dying trajectories is first proposed by",
        options: [
            "Glaser and Strauss",
            "Elisabeth Kubler-Ross",
            "Freud",
            "Erickson"
        ],
        a: 0 // Glaser and Strauss
    },
    {
        q: "Elisabeth Kubler-Ross wrote about the stages of death in her book",
        options: [
            "Research on Death",
            "On Death and Dying",
            "Perspectives on Dying",
            "Stages of Death"
        ],
        a: 1 // On Death and Dying
    },
    {
        q: "“If I’m allowed to live, I promise....” represents the",
        options: [
            "Denial stage",
            "Bargaining stage",
            "Depression stage",
            "Promise stage"
        ],
        a: 1 // Bargaining stage
    },
    {
        q: "The last stage of dying according to Ross is",
        options: [
            "Acceptance",
            "Denial",
            "Depression",
            "Bargaining"
        ],
        a: 0 // Acceptance
    },
    // Week 11 - Management of the Dying Patient
    {
        q: "The top cause of death now is",
        options: [
            "Heart disease",
            "Pneumonia",
            "Small pox",
            "Dengue"
        ],
        a: 0 // Heart disease
    },
    {
        q: "The full form of ADL is",
        options: [
            "Acute Disease of the Lung",
            "Activities of daily living",
            "Activities of daily life",
            "Acute disorder of liver"
        ],
        a: 1 // Activities of daily living
    },
    {
        q: "The top caregivers of the dead are their ___",
        options: [
            "Children",
            "Spouse",
            "Parent",
            "Friend"
        ],
        a: 0 // Children
    },
    {
        q: "Palliative care is a form of medical treatment focused on",
        options: [
            "providing a cure",
            "complete treatment without relapse",
            "slowing the disease's progress",
            "not doing any intervention"
        ],
        a: 2 // slowing the disease's progress
    },
    // Week 12 - Grief and Bereavement
    {
        q: "Acknowledgment of the objective fact that one has experienced a death is",
        options: [
            "Grief",
            "Loss",
            "Bereavement",
            "Trauma"
        ],
        a: 2 // Bereavement
    },
    {
        q: "The emotional response to that loss of a loved one is",
        options: [
            "Grief",
            "Loss",
            "Bereavement",
            "Trauma"
        ],
        a: 0 // Grief
    },
    {
        q: "Freud states that death of a loved one is",
        options: [
            "An emotional trauma",
            "A regression",
            "A defense mechanism",
            "Repression"
        ],
        a: 0 // An emotional trauma
    },
    {
        q: "Bowlby and Parkes presented ____ main stages in the grief process",
        options: [
            "Three",
            "Two",
            "Five",
            "Four"
        ],
        a: 3 // Four
    },
    {
        q: "The Dual Process Model of Grief and Loss was introduced by",
        options: [
            "Margaret Stroebe and Henk Schut",
            "Bowlby",
            "Jacobs",
            "Freud"
        ],
        a: 0 // Margaret Stroebe and Henk Schut
    },
    // Week 12 - Children and death – Children’s concepts of Death, Terminally ill Child, Helping Children Deal with Death
    {
        q: "Children",
        options: [
            "Have very good understanding of death",
            "Do not have appropriate understanding of death",
            "Have very bad understanding of death",
            "Have no understanding of death"
        ],
        a: 1 // Do not have appropriate understanding of death
    },
    {
        q: "Between the ages of 5 and 9, most children are beginning to realize that death is",
        options: [
            "Temporary",
            "Final",
            "Unrealistic",
            "Reversible"
        ],
        a: 1 // Final
    },
    {
        q: "When children question 'Why did they die?' Parents should say",
        options: [
            "The person's body couldn't fight the sickness any more",
            "God took the person to Him",
            "The person is very good",
            "The person is very bad"
        ],
        a: 0 // The person's body couldn't fight the sickness any more
    },
    {
        q: "Some studies have shown that when children experience the death of a close relative, such as a brother, sister, or parent, they often feel",
        options: [
            "Relieved",
            "Cheated",
            "Guilty",
            "Punishing"
        ],
        a: 2 // Guilty
    },
    {
        q: "If a grown-up child suddenly needs help eating and needs diapers, the child has",
        options: [
            "Been spoilt",
            "Regressed",
            "Acting",
            "Depressed"
        ],
        a: 1 // Regressed
    },
    // Week 12 - Concept of Compliance and predicting whether patients are compliant: The Work of Ley
    {
        q: "Which of the following is a compliant behavior?",
        options: [
            "Smoking",
            "Taking medication",
            "Maintaining under weight",
            "Self-medication"
        ],
        a: 1 // Taking medication
    },
    {
        q: "If the patient refuses to take appointment and meet doctor, then the patient is considered to be______",
        options: [
            "Compliant",
            "Noncompliant",
            "Adherent",
            "Partially compliant"
        ],
        a: 1 // Noncompliant
    },
    {
        q: "Which stage of compliance continuum results in taking suboptimal dose of medication?",
        options: [
            "Noncompliant",
            "Partially compliant",
            "Premature discontinuation",
            "Fully compliant"
        ],
        a: 2 // Premature discontinuation
    },
    {
        q: "Which of the following is NOT a predictor of compliance?",
        options: [
            "Characteristics of the illness",
            "Personal Characteristics",
            "Cultural Norms",
            "None of the above"
        ],
        a: 3 // None of the above
    },
    {
        q: "The cognitive hypothesis model of compliance offered by ________",
        options: [
            "Philip Ley",
            "Haynes",
            "Boyle",
            "Sara"
        ],
        a: 0 // Philip Ley
    },
    // Week 12 - Enhancing Compliance
    {
        q: "The effect of noncompliance is greater for ________",
        options: [
            "Chronic illness",
            "Acute illness",
            "Only cardiac related diseases",
            "Only cancer"
        ],
        a: 0 // Chronic illness
    },
    {
        q: "Which of the following measures of compliance is objective by nature?",
        options: [
            "Asking practitioner",
            "Asking patient",
            "Asking others",
            "Monitoring medicine usage"
        ],
        a: 3 // Monitoring medicine usage
    },
    {
        q: "Full form of “MEMS”",
        options: [
            "Medication Event Monitoring System",
            "Multiple Even Medication System",
            "Measure Event Monitoring System",
            "Medication Event Measuring System"
        ],
        a: 0 // Medication Event Monitoring System
    },
    {
        q: "Which of the following is NOT a strategy for enhancing patient’s compliance?",
        options: [
            "Educational strategy",
            "Criticizing the patient strategy",
            "Behavioral strategy",
            "Cognitive behavioral strategy"
        ],
        a: 1 // Criticizing the patient strategy
    },
    {
        q: "The SIMPLE strategy is practiced by the ________",
        options: [
            "Patient",
            "Practitioner",
            "Family members of the patient",
            "Pharmacist"
        ],
        a: 1 // Practitioner
    },
    // Week 13 - The role of information and Knowledge in illness and doctor–patient communication
    {
        q: "The _______________ suggested that patients’ beliefs are important and emphasized the patient’s locus of control, perceived social support and the disruption of lifestyle involved in adherence.",
        options: [
            "Information model",
            "Communication model",
            "Adherence model",
            "Layman’s model"
        ],
        a: 2 // Adherence model
    },
    {
        q: "The type of intervention that enables individuals to learn how the process or the intervention will actually be done is _______________",
        options: [
            "Behavioral instructions",
            "Sensory information",
            "Coping skills information",
            "Procedural information"
        ],
        a: 3 // Procedural information
    },
    {
        q: "One of the types of informational interventions which help a patient in recovery, by teaching the individual how to behave in terms of factors such as coughing and relaxing ___________________",
        options: [
            "Behavioral instructions",
            "Coping skills information",
            "Procedural information",
            "Sensory information"
        ],
        a: 0 // Behavioral instructions
    },
    {
        q: "The information which can educate the individual about possible coping strategies is ___________________",
        options: [
            "Behavioral instructions",
            "Coping skills information",
            "Procedural information",
            "Sensory information"
        ],
        a: 1 // Coping skills information
    },
    {
        q: "When the Doctor is giving instructions such as ‘I am going to tell you about three things . . . a, b and c’, and is using language and terms the patient can understand, the information being given is in ______________",
        options: [
            "Structured form",
            "Communication",
            "Instructions",
            "Comprehensiveness"
        ],
        a: 0 // Structured form
    },
    // Week 13 - The problem of doctor variability
    {
        q: "In their hypothetico-deductive model of problem solving, Newell and Simon differentiated between two types of plans:",
        options: [
            "Heuristics and algorithms",
            "Logic and technology",
            "Diagnosis and clinical correlation",
            "Problem statement and strategic planning"
        ],
        a: 0 // Heuristics and algorithms
    },
    {
        q: "Most human problem solving involves___________, which are rules of thumb",
        options: [
            "Solutions",
            "Heuristics",
            "Strategization",
            "Realism"
        ],
        a: 1 // Heuristics
    },
    {
        q: "Most meetings between health professionals and patients are time-limited and consequently ____________ play a central role in developing and testing a hypothesis and reaching a management decision.",
        options: [
            "Communication",
            "Compliance",
            "Adherence",
            "Stereotypes"
        ],
        a: 3 // Stereotypes
    },
    {
        q: "In a research study, either written or verbal _______________ must be taken, depending on the nature of the study and is an essential requirement for gaining ethical approval. Medical councils across the world stipulate that patients must be given sufficient information to enable them to consent to any procedure in an informed way.",
        options: [
            "Patient records",
            "Personal information",
            "Informed consent",
            "Demographic details"
        ],
        a: 2 // Informed consent
    },
    {
        q: "Literature has recommended ______________ as the preferred style of doctor–patient communication as a means to improve patient outcomes",
        options: [
            "Doctor centeredness",
            "Patient centeredness",
            "Medicine centeredness",
            "Hospital centeredness"
        ],
        a: 1 // Patient centeredness
    },
    // Week 13 - Significance of Placebo in Treatment
    {
        q: "Placebo originated from which Latin word?",
        options: [
            "I shall do",
            "I shall please",
            "I shall expect",
            "I shall pray"
        ],
        a: 1 // I shall please
    },
    {
        q: "An inert substance with no active medical properties is called",
        options: [
            "Morphine",
            "Surgery",
            "Paracetamol",
            "Saline solution"
        ],
        a: 3 // Saline solution
    },
    {
        q: "The patient’s belief that the treatment will have negative effect, even if the Medication given is completely inert is termed as:",
        options: [
            "Placebo effect",
            "Nocebo effect",
            "Side effect",
            "Both a & c"
        ],
        a: 1 // Nocebo effect
    },
    {
        q: "Who experimented on placebo treatment during World War II?",
        options: [
            "Henry Beecher",
            "John Hayghart",
            "Wright",
            "Saraphino"
        ],
        a: 0 // Henry Beecher
    }
    


];




    // ---------- Placeholder question groups (the actual groups you already have should be defined before this script runs) ----------
    // For demo if groups not present, create dummy groups
    const ensureDummyGroups = () => {
      if (typeof questions1 === 'undefined') {
        window.questions1 = [ { q:'Sample Q1 - group1', options:['A','B','C','D'], a:0 }, { q:'Sample Q2 - group1', options:['A','B','C','D'], a:1 } ];
        window.questions2 = [ { q:'Sample Q1 - group2', options:['A','B','C','D'], a:2 } ];
        for (let i=3;i<=13;i++){ window['quizQuestions'+i] = [{ q:`Sample Q1 - group${i}`, options:['A','B','C','D'], a:3 }]; }
      }
    };
    ensureDummyGroups();

    // ---------- Combine All Questions ----------
let questions = [
    ...questions1,
    ...questions2,
    ...questions3,
    ...questions4,
    ...quizQuestions5,
    ...quizQuestions6,
    ...quizQuestions7,
    ...quizQuestions8,
    ...quizQuestions9,
    ...quizQuestions10,
    ...quizQuestions11,
    ...quizQuestions12,
    ...quizQuestions13,
    ...quizQuestions14
];

// ---------- Group Variables (string keys) ----------
const groupVars = [
    "questions1",
    "questions2",
    "questions3",
    "questions4",
    "quizQuestions5",
    "quizQuestions6",
    "quizQuestions7",
    "quizQuestions8",
    "quizQuestions9",
    "quizQuestions10",
    "quizQuestions11",
    "quizQuestions12",
    "quizQuestions13",
    "quizQuestions14",
];

window.questions1 = questions1;
window.questions2 = questions2;
window.questions3 = questions3;
window.questions4 = questions4;
window.quizQuestions5 = quizQuestions5;
window.quizQuestions6 = quizQuestions6;
window.quizQuestions7 = quizQuestions7;
window.quizQuestions8 = quizQuestions8;
window.quizQuestions9 = quizQuestions9;
window.quizQuestions10 = quizQuestions10;
window.quizQuestions11 = quizQuestions11;
window.quizQuestions12 = quizQuestions12;
window.quizQuestions13 = quizQuestions13;
window.quizQuestions14 = quizQuestions14;

const groupLabels = groupVars.map((_, i) => `Group ${i + 1}`);

// ---------- State ----------
let masterGroups = {};
let currentQuestions = [];
let perPage = 10;
let currentPage = 1;
let totalPages = 1;
let quizAnswers = {};
let flagged = new Set();
let timerInterval = null;
let timerRemaining = 0;
let timerTotal = 0;

// ---------- Load Groups ----------
function loadMasterGroups() {
    masterGroups = {};

    groupVars.forEach(gv => {
        const arr = window[gv];
        if (Array.isArray(arr)) {
            masterGroups[gv] = arr.map(q => JSON.parse(JSON.stringify(q)));
        } else {
            masterGroups[gv] = [];
        }
    });
}
loadMasterGroups();

// ---------- UI References ----------
const groupList = document.getElementById('groupList');
const selectAllGroups = document.getElementById('selectAllGroups');
const startBtn = document.getElementById('startBtn');
const loadAllBtn = document.getElementById('loadAllBtn');
const quizArea = document.getElementById('quizArea');
const container = document.getElementById('questionsContainer');

const shuffleBtn = document.getElementById('shuffleBtn');
const checkBtn = document.getElementById('checkBtn');
const resetBtn = document.getElementById('resetBtn');
const showAnswersBtn = document.getElementById('showAnswersBtn');
const exportPdfBtn = document.getElementById('exportPdfBtn');

const prevPageBtn = document.getElementById('prevPage');
const nextPageBtn = document.getElementById('nextPage');
const currentPageSpan = document.getElementById('currentPage');
const totalPagesSpan = document.getElementById('totalPages');

const resultDiv = document.getElementById('result');
const progressSummary = document.getElementById('progressSummary');
const dotGrid = document.getElementById('dotGrid');
const flagJumpBtn = document.getElementById('flagJumpBtn');

const enableTimer = document.getElementById('enableTimer');
const timerSelect = document.getElementById('timerSelect');
const timerCustom = document.getElementById('timerCustom');
const timerWrapper = document.getElementById('timerWrapper');
const timerBar = document.getElementById('timerBar');
const timeText = document.getElementById('timeText');

const darkModeToggle = document.getElementById('darkModeToggle');

// ---------- Render Group List ----------
function renderGroupList() {
    groupList.innerHTML = "";

    groupLabels.forEach((label, idx) => {
        const gv = groupVars[idx];
        const div = document.createElement('div');
        div.className = 'form-check';

        div.innerHTML = `
            <input class="form-check-input" type="checkbox" id="grp_${idx}" data-gv="${gv}">
            <label class="form-check-label" for="grp_${idx}">
                ${label} (${masterGroups[gv].length})
            </label>
        `;

        groupList.appendChild(div);
    });
}
renderGroupList();

// ---------- Group Selection ----------
selectAllGroups.addEventListener('change', () => {
    groupList.querySelectorAll('input[type=checkbox]').forEach(cb => {
        cb.checked = selectAllGroups.checked;
    });
});

loadAllBtn.addEventListener('click', () => {
    groupList.querySelectorAll('input[type=checkbox]').forEach(cb => cb.checked = true);
});

// ---------- Start Quiz ----------
startBtn.addEventListener('click', () => {
    const selected = Array.from(
        groupList.querySelectorAll('input[type=checkbox]:checked')
    ).map(n => n.dataset.gv);

    if (!selected.length) {
        alert("Please select at least one group.");
        return;
    }

    currentQuestions = [];
    selected.forEach(gv => {
        masterGroups[gv].forEach(q =>
            currentQuestions.push(JSON.parse(JSON.stringify(q)))
        );
    });

    quizAnswers = JSON.parse(localStorage.getItem("quizAnswers_v2") || "{}");
    flagged = new Set(JSON.parse(localStorage.getItem("flagged_v2") || "[]"));

    currentPage = 1;
    perPage = 10; // keep pagination
    totalPages = Math.ceil(currentQuestions.length / perPage);
    updatePageIndicators();

    quizArea.style.display = 'block';
    renderQuestions();
    updateProgressUI();
});

// ─────────────────────────────────────────────
// ✅ TIMER SYSTEM
// ─────────────────────────────────────────────

function startTimer() {
    stopTimer();
    updateTimerUI();
    timerInterval = setInterval(() => {
        timerRemaining--;
        if (timerRemaining <= 0) {
            stopTimer();
            alert("Time is up — auto-submitting.");
            showResult();
        }
        updateTimerUI();
    }, 1000);
}

function stopTimer() {
    if (timerInterval) clearInterval(timerInterval);
    timerInterval = null;
}

function updateTimerUI() {
    const pct = Math.max(0, (timerRemaining / timerTotal) * 100);
    timerBar.style.width = pct + "%";

    const mm = String(Math.floor(timerRemaining / 60)).padStart(2, "0");
    const ss = String(timerRemaining % 60).padStart(2, "0");

    timeText.textContent = `${mm}:${ss}`;
}

// ─────────────────────────────────────────────
// ✅ PAGINATION
// ─────────────────────────────────────────────

prevPageBtn.addEventListener('click', () => {
    if (currentPage > 1) currentPage--;
    renderQuestions();
    updatePageIndicators();
});

nextPageBtn.addEventListener('click', () => {
    if (currentPage < totalPages) currentPage++;
    renderQuestions();
    updatePageIndicators();
});

function updatePageIndicators() {
    currentPageSpan.textContent = currentPage;
    totalPagesSpan.textContent = totalPages;
}

// ─────────────────────────────────────────────
// ⚡ SHUFFLE UTILITY
// ─────────────────────────────────────────────

function shuffleArray(arr) {
    return arr
        .map(v => ({ v, r: Math.random() }))
        .sort((a, b) => a.r - b.r)
        .map(o => o.v);
}

// ─────────────────────────────────────────────
// ✅ RENDER QUESTIONS (PATCHED FLAGGING)
// ─────────────────────────────────────────────

function renderQuestions() {
    container.innerHTML = "";

    if (!currentQuestions.length) return;

    const start = (currentPage - 1) * perPage;
    const end = Math.min(currentQuestions.length, start + perPage);

    for (let i = start; i < end; i++) {
        const item = currentQuestions[i];
        const localIdx = i;

        const qCard = document.createElement("div");
        qCard.className = "card mb-3";

        if (flagged.has(String(localIdx))) qCard.classList.add("flagged");

        const qBody = document.createElement("div");
        qBody.className = "card-body";

        const headerRow = document.createElement("div");
        headerRow.className =
            "d-flex justify-content-between align-items-center mb-2";

        const qTitle = document.createElement("h6");
        qTitle.className = "card-title mb-0";
        qTitle.textContent = `Q${localIdx + 1}. ${item.q}`;

        headerRow.appendChild(qTitle);

        const rightControls = document.createElement("div");
        rightControls.innerHTML = `
            <button type="button" class="btn btn-sm btn-outline-warning me-2 flagBtn">Flag</button>
        `;
        headerRow.appendChild(rightControls);

        qBody.appendChild(headerRow);

        // Render options (MCQ / match / alphalist)
        if (item.type === "match") {
            const table = document.createElement("table");
            table.className = "table table-bordered mt-2";

            table.innerHTML = `
                <thead>
                    <tr><th>List I</th><th>List II</th></tr>
                </thead>
            `;

            const tbody = document.createElement("tbody");
            const maxLen = Math.max(item.list1.length, item.list2.length);

            for (let r = 0; r < maxLen; r++) {
                const tr = document.createElement("tr");
                tr.innerHTML = `
                    <td>${item.list1[r] || ""}</td>
                    <td>${item.list2[r] || ""}</td>
                `;
                tbody.appendChild(tr);
            }
            table.appendChild(tbody);
            qBody.appendChild(table);

            const opts = document.createElement("div");
            opts.className = "mt-2";

            item.options.forEach((opt, optIdx) => {
                const optId = `q${localIdx}_opt${optIdx}`;
                const div = document.createElement("div");
                div.className = "form-check";

                const input = document.createElement("input");
                input.className = "form-check-input";
                input.type = "radio";
                input.name = `q${localIdx}`;
                input.id = optId;
                input.value = optIdx;

                if (quizAnswers[`q${localIdx}`] == optIdx) input.checked = true;

                input.addEventListener("change", () => {
                    quizAnswers[`q${localIdx}`] = optIdx;
                    persistAnswers();
                    updateDotForIndex(localIdx);
                    updateProgressUI();
                });

                const label = document.createElement("label");
                label.className = "form-check-label";
                label.htmlFor = optId;
                label.textContent = opt;

                div.appendChild(input);
                div.appendChild(label);
                opts.appendChild(div);
            });

            qBody.appendChild(opts);
        }

        else if (item.type === "alphalist") {

    // Show the list items (A, B, C, D...)
    const ul = document.createElement("ul");
    ul.className = "mt-2";

    (item.list1 || []).forEach(text => {
        const li = document.createElement("li");
        li.textContent = text;
        ul.appendChild(li);
    });

    qBody.appendChild(ul);

    // Render answer options
    if (item.options && item.options.length) {
        const opts = document.createElement("div");
        opts.className = "mt-2";

        item.options.forEach((opt, optIdx) => {
            const optId = `q${localIdx}_opt${optIdx}`;
            const div = document.createElement("div");
            div.className = "form-check";

            const input = document.createElement("input");
            input.className = "form-check-input";
            input.type = "radio";
            input.name = `q${localIdx}`;
            input.id = optId;
            input.value = optIdx;

            if (quizAnswers[`q${localIdx}`] == optIdx) input.checked = true;

            input.addEventListener("change", () => {
                quizAnswers[`q${localIdx}`] = optIdx;
                persistAnswers();
                updateDotForIndex(localIdx);
                updateProgressUI();
            });

            const label = document.createElement("label");
            label.className = "form-check-label option-label";
            label.htmlFor = optId;
            label.textContent = opt;

            div.appendChild(input);
            div.appendChild(label);
            opts.appendChild(div);
        });

        qBody.appendChild(opts);
    }
}

        // normal MCQ
        else {
            const opts = document.createElement("div");
            opts.className = "mt-2";

            item.options.forEach((opt, optIdx) => {
                const optId = `q${localIdx}_opt${optIdx}`;
                const div = document.createElement("div");
                div.className = "form-check";

                const input = document.createElement("input");
                input.className = "form-check-input";
                input.type = "radio";
                input.name = `q${localIdx}`;
                input.id = optId;
                input.value = optIdx;

                if (quizAnswers[`q${localIdx}`] == optIdx) input.checked = true;

                input.addEventListener("change", () => {
                    quizAnswers[`q${localIdx}`] = optIdx;
                    persistAnswers();
                    updateDotForIndex(localIdx);
                    updateProgressUI();
                });

                const label = document.createElement("label");
                label.className = "form-check-label option-label";
                label.htmlFor = optId;
                label.textContent = opt;

                div.appendChild(input);
                div.appendChild(label);

                opts.appendChild(div);
            });

            qBody.appendChild(opts);
        }

        qCard.appendChild(qBody);
        container.appendChild(qCard);

        // --- FLAG BUTTON (PATCHED, NO RESET) ---
        qCard.querySelector(".flagBtn").addEventListener("click", () => {
            const key = String(localIdx);

            if (flagged.has(key)) {
                flagged.delete(key);
                qCard.classList.remove("flagged");
            } else {
                flagged.add(key);
                qCard.classList.add("flagged");
            }

            persistFlags();
            updateDotForIndex(localIdx); // ONLY update one dot
        });
    }

    updateProgressUI();
}

// ─────────────────────────────────────────────
// ✅ ONLY UPDATE ONE DOT (fixes reset bug)
// ─────────────────────────────────────────────
function updateDotForIndex(i) {
    const dot = dotGrid.children[i];
    if (!dot) return;

    const key = String(i);
    dot.className = "question-dot";

    if (flagged.has(key)) dot.classList.add("dot-flagged");
    else if (quizAnswers[`q${i}`] !== undefined) dot.classList.add("dot-answered");
    else dot.classList.add("dot-unanswered");
}

// ─────────────────────────────────────────────
// Persist answers & flags
// ─────────────────────────────────────────────

function persistAnswers() {
    localStorage.setItem("quizAnswers_v2", JSON.stringify(quizAnswers));
}

function persistFlags() {
    localStorage.setItem("flagged_v2", JSON.stringify([...flagged]));
}

// ─────────────────────────────────────────────
// Progress & Dot Grid
// ─────────────────────────────────────────────

function updateProgressUI() {
    const total = currentQuestions.length;
    const answered = Object.keys(quizAnswers).length;

    progressSummary.textContent =
        `Progress: ${answered} / ${total} answered • Flagged: ${flagged.size}`;

    dotGrid.innerHTML = "";

    for (let i = 0; i < currentQuestions.length; i++) {
        const dot = document.createElement("span");
        dot.className = "question-dot";
        dot.title = `Q${i + 1}`;

        if (flagged.has(String(i))) dot.classList.add("dot-flagged");
        else if (quizAnswers[`q${i}`] !== undefined) dot.classList.add("dot-answered");
        else dot.classList.add("dot-unanswered");

        dot.addEventListener("click", () => {
            currentPage = Math.floor(i / perPage) + 1;
            renderQuestions();
            updatePageIndicators();
            window.scrollTo({ top: 0, behavior: "smooth" });
        });

        dotGrid.appendChild(dot);
    }
}

// ─────────────────────────────────────────────
// Score, Show Results, Correct Answers
// ─────────────────────────────────────────────

function calculateScore() {
    let score = 0;

    currentQuestions.forEach((q, i) => {
        const chosen = quizAnswers[`q${i}`];
        if (chosen !== undefined && Number(chosen) === q.a) score++;
    });

    return { score, total: currentQuestions.length };
}

function showResult() {
    const res = calculateScore();
    const pct = Math.round((res.score / res.total) * 100);

    resultDiv.style.display = "block";
    resultDiv.innerHTML = `
        <div class="alert alert-primary">
            You scored <strong>${res.score}</strong> of <strong>${res.total}</strong> (${pct}%)
        </div>
        <button id="revealWrong" class="btn btn-sm btn-outline-danger">Show Answers</button>
        <button id="closeRes" class="btn btn-sm btn-outline-secondary">Close</button>
    `;

    document.getElementById("revealWrong").addEventListener("click", showCorrectAnswers);
    document.getElementById("closeRes").addEventListener("click", () => {
        resultDiv.style.display = "none";
    });
}

function showCorrectAnswers() {
    const start = (currentPage - 1) * perPage;
    const end = Math.min(currentQuestions.length, start + perPage);

    for (let i = start; i < end; i++) {
        const card = container.children[i - start];
        const q = currentQuestions[i];

        const inputs = card.querySelectorAll("input");

        inputs.forEach((inp, idx) => {
            const label = card.querySelector(`label[for=${inp.id}]`);
            label.classList.remove("text-success", "text-danger", "fw-bold");

            if (idx === q.a) {
                label.classList.add("text-success", "fw-bold");
                inp.checked = true;
            } else if (inp.checked) {
                label.classList.add("text-danger");
            }
        });
    }
}

// ─────────────────────────────────────────────
// Reset
// ─────────────────────────────────────────────

function resetQuiz() {
    if (!confirm("Reset your quiz answers?")) return;
    quizAnswers = {};
    flagged = new Set();
    persistAnswers();
    persistFlags();
    renderQuestions();
    updateProgressUI();
    resultDiv.style.display = "none";
}

// ─────────────────────────────────────────────
// Shuffle
// ─────────────────────────────────────────────

shuffleBtn.addEventListener("click", () => {
    quizAnswers = {};
    flagged = new Set();
    persistAnswers();
    persistFlags();

    currentQuestions = shuffleArray(currentQuestions);

    currentQuestions.forEach(q => {
        if (Array.isArray(q.options)) {
            const correct = q.options[q.a];
            q.options = shuffleArray(q.options);
            q.a = q.options.indexOf(correct);
        }
    });

    currentPage = 1;
    totalPages = Math.ceil(currentQuestions.length / perPage);
    updatePageIndicators();

    renderQuestions();
});

// ─────────────────────────────────────────────
// Button Events
// ─────────────────────────────────────────────

checkBtn.addEventListener("click", showResult);
resetBtn.addEventListener("click", resetQuiz);
showAnswersBtn.addEventListener("click", showCorrectAnswers);

flagJumpBtn.addEventListener("click", () => {
    if (flagged.size === 0) return alert("No flagged questions.");

    const first = [...flagged][0];
    currentPage = Math.floor(first / perPage) + 1;

    renderQuestions();
    updatePageIndicators();
    window.scrollTo({ top: 0, behavior: "smooth" });
});

// ─────────────────────────────────────────────
// Export PDF
// ─────────────────────────────────────────────

exportPdfBtn.addEventListener("click", async () => {
    const res = calculateScore();

    const exportEl = document.createElement("div");
    exportEl.style.padding = "20px";
    exportEl.style.background = "#fff";

    exportEl.innerHTML = `
        <h3>Quiz Results</h3>
        <p>Score: ${res.score} / ${res.total}</p>
        <hr>
    `;

    currentQuestions.forEach((q, i) => {
        const chosen = quizAnswers[`q${i}`] !== undefined
            ? q.options[quizAnswers[`q${i}`]]
            : "---";

        const correct = q.options[q.a];

        exportEl.innerHTML += `
            <p>
                <strong>Q${i + 1}.</strong> ${q.q}<br>
                Selected: ${chosen}<br>
                Correct: ${correct}
            </p>
        `;
    });

    document.body.appendChild(exportEl);

    const canvas = await html2canvas(exportEl, { scale: 1.5 });
    const imgData = canvas.toDataURL("image/png");

    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF("p", "pt", "a4");

    const pageWidth = pdf.internal.pageSize.getWidth();
    const imgProps = pdf.getImageProperties(imgData);
    const imgWidth = pageWidth - 40;
    const imgHeight = (imgProps.height * imgWidth) / imgProps.width;

    pdf.addImage(imgData, "PNG", 20, 20, imgWidth, imgHeight);
    pdf.save("quiz-results.pdf");

    document.body.removeChild(exportEl);
});

// ─────────────────────────────────────────────
// DARK MODE
// ─────────────────────────────────────────────

darkModeToggle.addEventListener("change", () => {
    const theme = darkModeToggle.checked ? "dark" : "light";
    document.documentElement.setAttribute("data-bs-theme", theme);
    localStorage.setItem("quiz_theme", theme);
});

(function () {
    const t = localStorage.getItem("quiz_theme") || "light";
    document.documentElement.setAttribute("data-bs-theme", t);
    darkModeToggle.checked = t === "dark";
})();

// Initial page count
totalPages = 1;
updatePageIndicators();


    // load any existing groups again (in case global vars changed)
    //loadMasterGroups(); renderGroupList();